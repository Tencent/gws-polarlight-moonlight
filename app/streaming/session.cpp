...
FloatingBall *g_ball = nullptr;

CWebConfig *g_pConfig1 = nullptr;
CWebConfig *g_pConfig2 = nullptr;
CWebConfig *g_pConfig3 = nullptr;

std::vector<char> vecCliGuuid;
enum emSDL_Screen_Event {
  SDL_NOTIC_SINGLE_SCREEN = 1,
  SDL_NOTIC_DOUBLE_SCREEN = 2,
};
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode == HC_ACTION) {
    ...
    if (wParam == WM_KEYDOWN && kbs->vkCode == 'D' && (GetAsyncKeyState(VK_SHIFT) & 0x8000) &&
      ...
      // MessageBox(NULL, L"快捷键触发!", L"通知", MB_OK);
...
void Session::SetDisplayCount(int n, int status) {
  return;
  if (m_isDualDisplay == true) {
    if (status == 1) {
      Session::m_DisConnectErrorCode = 1;

    } else {
      if (n == 1) {
        qDebug() << "SetDisplayCount ";
        Session::m_DisConnectErrorCode = 1;
      }
    }
  }
...
void Session::clStageFailed(int stage, int errorCode) {
  qDebug() << "clStageFailed Session::clStageFailed = " << s_ActiveSession->n_RtspTryAgain;
  if (s_ActiveSession->n_RtspTryAgain == 0) {
    return;
  }
...
void Session::clConnectionTerminated(int errorCode) {
  unsigned int portFlags = LiGetPortFlagsFromErrorCode(errorCode);
  s_ActiveSession->m_PortTestResults = LiTestClientConnectivity(CONN_TEST_SERVER, 443, portFlags);
  ...
  switch (errorCode) {
    ...
    case ML_ERROR_NO_VIDEO_TRAFFIC:
      ...
      emit s_ActiveSession->displayLaunchError(
        tr("No video received from host.") + "\n\n" +
    ...
    case ML_ERROR_NO_VIDEO_FRAME:
      s_ActiveSession->m_UnexpectedTermination = true;
      emit s_ActiveSession->displayLaunchError(
        tr("Your network connection isn't performing well. Reduce your video "
           "bitrate setting or try a faster connection."));
      break;
    ...
    case ML_ERROR_UNEXPECTED_EARLY_TERMINATION:
      ...
      emit s_ActiveSession->displayLaunchError(tr("Something went wrong on your host PC when starting the stream.") +
                                               "\n\n" +
                                               tr("Make sure you don't have any DRM-protected content open on your "
                                                  "host PC. You can also try restarting your host PC."));
      break;

    case ML_ERROR_FRAME_CONVERSION:
      s_ActiveSession->m_UnexpectedTermination = true;
      emit s_ActiveSession->displayLaunchError(tr("The host PC reported a fatal video encoding error.") + "\n\n" +
                                               tr("Try disabling HDR mode, changing the streaming resolution, or "
                                                  "changing your host PC's display resolution."));
      break;
  ...
  if (m_DisConnectErrorCode == 0) {
    m_DisConnectErrorCode = errorCode;
  }
  SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Connection terminated: %d", m_DisConnectErrorCode);
  ...
  switch (connectionStatus) {
    case CONN_STATUS_POOR:
      if (s_ActiveSession->m_StreamConfig.bitrate > 5000) {
        strcpy(s_ActiveSession->m_OverlayManager.getOverlayText(Overlay::OverlayStatusUpdate),
...
Session::Session(NvComputer *computer, NvApp &app, StreamingPreferences *preferences)
  ...
  g_pConfigPopup = nullptr;
  m_checkDisplayDual = NULL;
}
...
bool Session::initialize() {
  ...
  /* QProcess p(NULL);
      ...
      p.waitForStarted();
      p.waitForFinished();
      m_ClientArch = QString::fromLocal8Bit(p.readAllStandardOutput());
      p.close();
  */
#endif
  qInfo() << "m_ClientOS:" << m_ClientOS;
  qInfo() << "m_ClientArch:" << m_ClientArch;
  qDebug() << "m_isDualDisplay = " << m_isDualDisplay;
  InitWebConfig();
  return true;
}
...
class DeferredSessionCleanupDisplay2Task : public QRunnable {
  public:
  DeferredSessionCleanupDisplay2Task(Session *session) : m_Session(session) {}

  private:
  virtual ~DeferredSessionCleanupDisplay2Task() override {}

  void run() override {
    // Finish cleanup of the connection state
    LiStopConnection2();
    if (m_Session->m_metricsReportTask) {
      m_Session->m_metricsReportTask->Restart();
    }
  }
...
class CheckDisplayDualTask : public QThread {
  public:
  CheckDisplayDualTask() : QThread(nullptr) {
    setObjectName("CheckDisplayDualTask");
    m_bRun = false;
    m_bWhileRun = true;
  }
  void SetSession(Session *session) {
    qDebug() << "CheckDisplayDualTask SetSession ";
    m_Session = session;
  }
  void Stop() { m_bWhileRun = false; }
  ...
  void run() override {
    ...
    int nValue = -1;
    qDebug() << "m_bRune = " << m_bRun;
    while (m_bWhileRun) {
      QThread::msleep(1000);
      if (m_bRun == false) {
        continue;
      }
      // qDebug() << "CheckDisplayDualTask::run nValue = " << nValue << " m_Session->m_isDualDisplay" <<
      // m_Session->m_isDualDisplay;
      try {
        nValue = http.CheckDisplay();
        if (nValue == 0)  // single
        {
          if (m_Session->m_isDualDisplay == true) {
            m_Session->m_isDualDisplay = false;
            SDL_Event event;
            event.type = SDL_NOTIC_SINGLE_SCREEN;
            event.quit.timestamp = SDL_GetTicks();
            SDL_PushEvent(&event);
          }
        } else if (nValue == 1)  // Dual
        {
          if (m_Session->m_isDualDisplay == false) {
            SDL_Event event;
            event.type = SDL_NOTIC_DOUBLE_SCREEN;
            event.quit.timestamp = SDL_GetTicks();
            SDL_PushEvent(&event);
            m_bRun = false;
          }
...
class DeferredSessionCleanupTask : public QRunnable {
  ...
  void run() override {
    qDebug() << " DeferredSessionCleanupTask run Session::m_DisConnectErrorCode = " << Session::m_DisConnectErrorCode;
    ...
    if (nStopExit <= 0) {
      if (Session::m_DisConnectErrorCode == 1) {
#ifdef Q_OS_WIN32
        char szExc[258] = {0};
        sprintf_s(szExc, "Tip.exe \"Gleam restarting......\"");
        WinExec(szExc, SW_SHOW);
#else
        std::string ExcuteStr = "open -a Tip.app --args \"Gleam restarting.\"";
...
#endif
      } else if (Session::m_DisConnectErrorCode == -1) {
#ifdef Q_OS_WIN32
        // The network is disconnected abnormally, please reconnect.
        char szExc[258] = {0};
        sprintf_s(szExc, "Tip.exe \"The network is disconnected abnormally.<br>restart ...\"");
        WinExec(szExc, SW_SHOW);
#else
        std::string ExcuteStr = "open -a Tip.app --args \"The network is disconnected abnormally.<br> restart .\"";
        qDebug() << "ExcuteStr = " << ExcuteStr;
        FILE *handle = popen(ExcuteStr.c_str(), "w");
        if (handle) {
          pclose(handle);
...
#endif
        qDebug() << "network close client nStopExit = " << nStopExit;
...
void Session::mactoggleFullscreen() {
#ifdef Q_OS_DARWIN
  if (StreamingPreferences::WM_FULLSCREEN_DESKTOP == m_Preferences->windowMode) {
    return;
  }
...
void Session::toggleFullscreen() {
  ...
  if (m_isDualDisplay) {
    SDL_SetWindowFullscreen(m_Window2, fullScreen ? m_FullScreenFlag : 0);
  }
#ifdef Q_OS_DARWIN
...
bool Session::startConnectionAsync() {
  ...
  try {
    ...
    http.startApp(m_Computer->currentGameId != 0 ? "resume" : "launch", m_Computer->isNvidiaServerSoftware, m_App.id,
                  &m_StreamConfig, enableGameOptimizations, m_Preferences->playAudioOnHost,
                  m_InputHandler->getAttachedGamepadMask(), !m_Preferences->multiController, rtspSessionUrl,
                  rtspCSessionID, m_ClientUUid, guuidEncryptText);
  } catch (const GfeHttpResponseException &e) {
    qDebug() << "startConnectionAsync error code " << e.toQString().toStdString();
    std::string szErrorCode = e.toQString().toStdString();
    if (szErrorCode == std::string(" (Error 401)")) {
      emit displayLaunchError(tr("The client is not authorized"));
    } else if (szErrorCode == std::string(" (Error 461)")) {
      emit displayLaunchError(tr("Missing client UUID info, reinstall client may solve the issue."));
    } else if (szErrorCode == std::string(" (Error 462)")) {
      emit displayLaunchError(tr("Client encryption key not found, reinstall client may solve the issue."));
    } else if (szErrorCode == std::string(" (Error 503)")) {
      emit displayLaunchError(tr("Host is busy or in use, limit has been reach for active streaming sessions."));
    } else {
  ...
  if (m_ClientUUid.size() > 0 && m_PrivateKeyStr.size() > 0 && guuidEncryptText.size() == 0) {
    qInfo() << "sunshine encrypt version error" << m_ClientUUid.size() << " ," << m_PrivateKeyStr.size() << " ,"
            << guuidEncryptText.size();
    qInfo() << "nRtspStatus =" << nRtspStatus;
    /*
#ifdef Q_OS_WIN32
    char szExc[258] = { 0 };
  ...
  if (guuidEncryptText.size() > 0) {
    ...
    RsaPriDecrypt(newStr.data(), newStr.size(), m_PrivateKeyStr, dencrypt_vec);  // private key decrypt
    vecCliGuuid.assign(dencrypt_vec.begin(), dencrypt_vec.end());
  }
  qInfo() << "vecCliGuuid.size()" << vecCliGuuid.size();
  int err = LiStartConnection(m_isDualDisplay ? 2 : 1, &hostInfo, &m_StreamConfig, &k_ConnCallbacks, &m_VideoCallbacks,
                              m_AudioDisabled ? nullptr : &m_AudioCallbacks, NULL, 0, NULL, 0);

  if (err != 0) {
    // We already displayed an error dialog in the stage failure
    // listener.
    qInfo() << "LiStartConnection error:" << err;
    return false;
  }

  emit connectionStarted();
  qInfo() << "RequestComputerInfor:" << rtspCSessionID;
  RequestComputerInfor(rtspCSessionID);
  return true;
}
...
// Button states
enum ButtonState { BUTTON_NORMAL, BUTTON_HOVER, BUTTON_PRESSED };
...
void handleButtonEvent(Button &button, SDL_Event &e) {
...
void renderButton(SDL_Renderer *renderer, Button &button) {
  // Set button color based on its current state
  switch (button.state) {
    case BUTTON_NORMAL:
      SDL_SetRenderDrawColor(renderer, 192, 192, 192, 255);  // Semi-transparent gray
      break;
    case BUTTON_HOVER:
      SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);  // Opaque white
      break;
    case BUTTON_PRESSED:
      SDL_SetRenderDrawColor(renderer, 128, 128, 128, 255);  // Opaque dark gray
      break;
  }
...
void Session::execInternal() {
  ...
  if (!m_AsyncConnectionSuccess && n_RtspTryAgain == 0) {
    // try once again
    n_RtspTryAgain = 1;
    qDebug() << "rtsp handshake errpr try again";
    initialize();
    AsyncConnectionStartThread af_asyncConnThread(this);
    if (!m_ThreadedExec) {
      // Kick off the async connection thread while we sit here and pump the event loop
      af_asyncConnThread.start();
      while (!af_asyncConnThread.wait(10)) {
        QCoreApplication::processEvents(QEventLoop::ExcludeUserInputEvents);
        QCoreApplication::sendPostedEvents();
      }
  ...
  // MetricsReportTask metricsReportTask(this);
  // metricsReportTask.start();
  if (m_metricsReportTask == NULL) {
    m_metricsReportTask = new MetricsReportTask(this);
  }
  if (!m_metricsReportTask) {
    m_metricsReportTask->start();
  }

  if (m_checkDisplayDual == NULL) {
    m_checkDisplayDual = new CheckDisplayDualTask();
    m_checkDisplayDual->SetSession(this);
    m_checkDisplayDual->start();
  }
  int x, y, width, height;
  int display_index_1st = -1;
  getWindowDimensions(m_Window, x, y, width, height, display_index_1st);
  ...
  // If we're starting in windowed mode and the Gleam GUI is maximized,
  ...
  m_Window = SDL_CreateWindow("Gleam", x, y, width, height, defaultWindowFlags | StreamUtils::getPlatformWindowFlags());
  if (!m_Window) {
    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "SDL_CreateWindow() failed with platform flags: %s", SDL_GetError());

    m_Window = SDL_CreateWindow("Gleam", x, y, width, height, defaultWindowFlags);
  }

  if (m_isDualDisplay) {
    const char *title2 = "Gleam-2";
    if (m_IsFullScreen) {
      getWindowDimensions(m_Window2, x, y, width, height, display_index_1st);
    } else {
      x = margin_x + width;
      y = margin_y;
    }

    m_Window2 = SDL_CreateWindow(title2, x, y, width, height,
                                 defaultWindowFlags | StreamUtils::getPlatformWindowFlags());
    if (!m_Window2) {
      SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "SDL_CreateWindow() failed with platform flags: %s", SDL_GetError());

      m_Window2 = SDL_CreateWindow(title2, x, y, width, height, defaultWindowFlags);
    }
  }
  ...
  auto winID1 = SDL_GetWindowID(m_Window);
  int winID2 = -1;
  if (m_isDualDisplay) winID2 = SDL_GetWindowID(m_Window2);

// HACK: Remove once proper Dark Mode support lands in SDL
#ifdef Q_OS_WIN32
  auto set_dark_mode = [&](SDL_Window *pWindow) {
    BOOL darkModeEnabled = FALSE;

    // Query whether dark mode is enabled for our Qt window (which tracks the OS dark mode state)
    QWindowList windows = QGuiApplication::topLevelWindows();
    for (const QWindow *window : windows) {
      if (SUCCEEDED(DwmGetWindowAttribute((HWND)window->winId(), DWMWA_USE_IMMERSIVE_DARK_MODE, &darkModeEnabled,
                                          sizeof(darkModeEnabled))) ||
          SUCCEEDED(DwmGetWindowAttribute((HWND)window->winId(), DWMWA_USE_IMMERSIVE_DARK_MODE_OLD, &darkModeEnabled,
                                          sizeof(darkModeEnabled)))) {
        break;
      }
    }
    ...
    if (SDL_GetWindowWMInfo(pWindow, &info) && info.subsystem == SDL_SYSWM_WINDOWS) {
      RECT clientRect;
      HBRUSH blackBrush;
      ...
      if (darkModeEnabled) {
        if (FAILED(DwmSetWindowAttribute(info.info.win.window, DWMWA_USE_IMMERSIVE_DARK_MODE, &darkModeEnabled,
                                         sizeof(darkModeEnabled)))) {
          DwmSetWindowAttribute(info.info.win.window, DWMWA_USE_IMMERSIVE_DARK_MODE_OLD, &darkModeEnabled,
                                sizeof(darkModeEnabled));
        }
  ...
  set_dark_mode(m_Window);
  if (m_isDualDisplay) {
    set_dark_mode(m_Window2);
  }
#endif
  ...
  SDL_Surface *iconSurface = SDL_CreateRGBSurfaceWithFormatFrom((void *)svgImage.constBits(), svgImage.width(),
                                                                svgImage.height(), 32, 4 * svgImage.width(),
                                                                SDL_PIXELFORMAT_RGBA32);
  SDL_Surface *iconSurface2 = nullptr;
  if (m_isDualDisplay) {
    iconSurface2 = SDL_CreateRGBSurfaceWithFormatFrom((void *)svgImage.constBits(), svgImage.width(), svgImage.height(),
                                                      32, 4 * svgImage.width(), SDL_PIXELFORMAT_RGBA32);
  }
#ifndef Q_OS_DARWIN
  ...
  if (m_isDualDisplay /* && iconSurface2 != nullptr*/) {
    SDL_Surface *icon = SDL_LoadBMP("Gleam_wix.png");
    if (icon) {
  ...
  if (m_isDualDisplay) updateOptimalWindowDisplayMode(m_Window2);

  // Enter full screen if requested
  if (m_IsFullScreen) {
    SDL_SetWindowFullscreen(m_Window, m_FullScreenFlag);
    if (m_isDualDisplay) SDL_SetWindowFullscreen(m_Window2, m_FullScreenFlag);
  ...
  int currentDisplayIndex2 = -1;
  if (m_isDualDisplay) currentDisplayIndex2 = SDL_GetWindowDisplayIndex(m_Window2);
  ...
  int nRet = SDL_RegisterEvents(SDL_NOTIC_SINGLE_SCREEN);
  if (nRet == ((uint32_t)-1)) {
    qInfo() << "create SDL_NOTIC_SINGLE_SCREEN event failue";
  }
  nRet = SDL_RegisterEvents(SDL_NOTIC_DOUBLE_SCREEN);
  if (nRet == ((uint32_t)-1)) {
    qInfo() << "create SDL_NOTIC_DOUBLE_SCREEN event failue";
  }

  /*
  if(!g_ball)
  {
      g_ball = new FloatingBall();
      g_ball->move(100,100);
      g_ball->init();
      g_ball->show();
      qDebug()<<"g_ball "<< x<<" "<<y;
      ...
      SDL_VERSION(&wmInfo.version);  // Initialize wmInfo to the version SDL compiled against

      if (SDL_GetWindowWMInfo(m_Window, &wmInfo)) {
          HWND hwnd = wmInfo.info.win.window;  // Here is your HWND
          // You can now use hwnd with Windows API functions
          SetParent((HWND)g_ball->winId(), hwnd);
      }
#endif
  }
*/
  ...
  // Button button;
  // initButton(button, BUTTON_X, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT);
  qDebug() << "needsFirstEnterCapture =  " << needsFirstEnterCapture;
#ifdef TARGET_OS_MAC
  ...
  for (;;) {
    ...
    if (!SDL_WaitEventTimeout(&event, 1000000)) {  // Victor
...
#  endif
    ...
    switch (event.type) {
      case SDL_NOTIC_SINGLE_SCREEN: {
#if 1
        qInfo() << "SDL_NOTIC_SINGLE_SCREEN";
        m_InputHandler->raiseAllKeys();
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Quit SDL_NOTIC_SINGLE_SCREEN");
        ...
        if (iconSurface2 != nullptr) {
          qInfo() << "free iconSurface2";
          SDL_FreeSurface(iconSurface2);
          iconSurface2 = NULL;
        }
        ...
        QThreadPool::globalInstance()->start(new DeferredSessionCleanupDisplay2Task(this));

        SetStremNum(1);
        m_isDualDisplay = false;
#endif
      } break;
      case SDL_NOTIC_DOUBLE_SCREEN: {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Quit SDL_NOTIC_DOUBLE_SCREEN");
        ...
        QApplication::exit(emExitRestart::RETCODE_RESTART);
        Session::m_DisConnectErrorCode = 1001;
      ...
      case SDL_WINDOWEVENT:
        ...
        if (event.window.event != SDL_WINDOWEVENT_SIZE_CHANGED && event.window.event != SDL_WINDOWEVENT_SHOWN) {
          ...
          if (m_isDualDisplay) {
            if (SDL_GetWindowDisplayIndex(m_Window2) == currentDisplayIndex2) {
              break;
            }
          }
#endif
        ...
        if (m_FlushingWindowEventsRef > 0) {
          ...
          SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Dropping window event during flush: %d (%d %d)",
                      event.window.event, event.window.data1, event.window.data2);
          break;
        }

        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Recreating renderer for window event: %d (%d %d)",
                    event.window.event, event.window.data1, event.window.data2);
      ...
      case SDL_RENDER_TARGETS_RESET:
        ...
        if (event.type != SDL_WINDOWEVENT) {
          SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Recreating renderer by internal request: %d", event.type);
        }
        ...
        if (m_isDualDisplay) {
          if (event.window.windowID == winID2) {
            if (currentDisplayIndex2 != SDL_GetWindowDisplayIndex(m_Window2)) {
              currentDisplayIndex2 = SDL_GetWindowDisplayIndex(m_Window2);
...

        {
          ...
          if (m_VideoDecoder == nullptr || event.window.windowID == winID1) {
            if (!chooseDecoder(m_Preferences->videoDecoderSelection, m_Window, m_ActiveVideoFormat, m_ActiveVideoWidth,
                               m_ActiveVideoHeight, m_ActiveVideoFrameRate, enableVsync,
                               enableVsync && m_Preferences->framePacing, false, s_ActiveSession->m_VideoDecoder)) {
              SDL_AtomicUnlock(&m_DecoderLock);
          ...
          if ((m_isDualDisplay && m_VideoDecoder2 == nullptr) || event.window.windowID == winID2) {
            if (m_isDualDisplay) {
              if (!chooseDecoder(m_Preferences->videoDecoderSelection, m_Window2, m_ActiveVideoFormat,
                                 m_ActiveVideoWidth, m_ActiveVideoHeight, m_ActiveVideoFrameRate, enableVsync,
                                 enableVsync && m_Preferences->framePacing, false, s_ActiveSession->m_VideoDecoder2)) {
                SDL_AtomicUnlock(&m_DecoderLock);
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to recreate decoder2 after reset");
                emit displayLaunchError(
                  tr("Unable to initialize video decoder. Please check your streaming settings and again."));
                goto DispatchDeferredCleanup;
              } else {
        ...
        if (event.window.windowID == winID1 && m_VideoDecoder != nullptr) {
          m_VideoDecoder->setHdrMode(LiGetCurrentHostDisplayHdrMode());
        }
        if (m_isDualDisplay) {
          if (event.window.windowID == winID2 && m_VideoDecoder2 != nullptr) {
...
DispatchDeferredCleanup:
  qDebug() << " DispatchDeferredCleanup Session::m_DisConnectErrorCode = " << Session::m_DisConnectErrorCode;
#ifdef _WIN64
  UnregisterHotkey();
#endif
  QtWebViewClose();
  ...
  m_checkDisplayDual->Stop();
  m_checkDisplayDual->quit();
  m_checkDisplayDual->wait();
  ...
  */
  ...
  if (event.window.windowID == winID1) {
    delete m_VideoDecoder;
    m_VideoDecoder = nullptr;
  } else if (event.window.windowID == winID2) {
    if (m_VideoDecoder2) {
      delete m_VideoDecoder2;
      m_VideoDecoder2 = nullptr;
    }
  ...
  if (/*event.window.windowID == winID1 &&*/ m_Window) {
    SDL_DestroyWindow(m_Window);
    m_Window = NULL;
  }

  if (/*event.window.windowID == winID2 &&*/ m_Window2) {
    SDL_DestroyWindow(m_Window2);
    m_Window2 = NULL;
  }
  // else {
  ...
  if (iconSurface2 != nullptr) {
    SDL_FreeSurface(iconSurface2);
    iconSurface2 = nullptr;
  }
  CloseSetting();
  CloseConfigSetting();
  QThreadPool::globalInstance()->start(new DeferredSessionCleanupTask(this));
  SDL_QuitSubSystem(SDL_INIT_VIDEO);
  if (Session::m_DisConnectErrorCode == 1001) {
    QApplication::exit(emExitRestart::RETCODE_RESTART);
  } else {
    QApplication::exit(0);
  }
...
void Session::CheckDual() {
  // if(m_isDualDisplay == false)
  { m_checkDisplayDual->Restart(); }
}
std::string Session::parse(QString json, QString csessionid, QString &httpCurl, QString &token,
  ...
  subObj.insert("clientOS", m_ClientOS);
  subObj.insert("clientArch", m_ClientArch);

  QJsonObject sessionid;
  sessionid.insert("sessionId", csessionid);
  sessionid.insert("attributes", subObj);

  // body.insert("body",attributes);
...
void Session::SetComputerInfor(const std::string &Curl, const std::string &Token, const std::string &ClientArch,
                               const std::string &ClientUUid, const std::string &privateKeyStr,
  ...
  if (m_ClientArch.isEmpty()) {
    m_ClientArch = ClientArch.c_str();
  }
  m_IsFullScreen = IsFullScreen;
}
void Session::RequestComputerInfor(QString Csessionid) {
  QString szUrl;
  QString strtoken;
  QJsonDocument retJson;
  std::string body = parse(m_HttpJsonStr.c_str(), Csessionid, szUrl, strtoken, retJson);
  if (m_HttpJsonStr.size() > 0 && Csessionid.size() > 0 && szUrl.size() > 0 && strtoken.size() > 0) {
    NvPairingManager pairingManager(m_Computer);
    pairingManager.sendSelfComputerInforToGws(szUrl, strtoken, retJson);
  }
}
