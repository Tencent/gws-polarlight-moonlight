...
void SdlInputHandler::performSpecialKeyCombo(KeyCombo combo)
{
    ...
    case KeyComboToggleFullScreen_1:
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Detected full-screen toggle combo 1");
        setCaptureActive(false);
        Session::s_ActiveSession->toggleFullscreen(1);
        ...
        break;
    case KeyComboToggleFullScreen_2:
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Detected full-screen toggle combo2");
        setCaptureActive(false);
        Session::s_ActiveSession->toggleFullscreen(2);
    ...
    case KeyComboToggleStatsOverlay:
        ...
        Session::get()->getOverlayManager(m_Window).setOverlayState(Overlay::OverlayDebug,
                                                                    !Session::get()->getOverlayManager(m_Window).isOverlayEnabled(Overlay::OverlayDebug));
        if (m_Window2) {
            Session::get()->getOverlayManager(m_Window2).setOverlayState(Overlay::OverlayDebug,
                                                                         !Session::get()->getOverlayManager(m_Window2).isOverlayEnabled(Overlay::OverlayDebug));
        }
        break;
    ...
    case KeyComboPasteText:
    {
        ...
        if (SDL_HasClipboardText() && (text = SDL_GetClipboardText()) != nullptr) {
            ...
            if(strlen(text) > 32000) {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    "Text in clipboard is too large to paste!");
            } else {
    ...
    //     break;
    case KeyComboStreamConfigWebView:
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "show setting");
        // Stop handling future input
        setCaptureActive(false);
        raiseAllKeys();
        ShowSetting();
    ...
    break;
    case KeyComboSettingWebView:
    {
        setCaptureActive(false);
        raiseAllKeys();
        ShowGleamWebViewSetting();
    }
    break;
    case KeyComboHelpWebViewQuestion:
    case KeyComboHelpWebView:
    {
        setCaptureActive(false);
        raiseAllKeys();
        ShowHelpWebView();
...
void SdlInputHandler::setLastActiveWindowIndex(SDL_KeyboardEvent* event) {
    if (event->windowID == SDL_GetWindowID(m_Window)) {
        lastActiveWindowIndex = 1;
    } else if (event->windowID == SDL_GetWindowID(m_Window2)) {
        lastActiveWindowIndex = 2;
    }
}

void SdlInputHandler::raiseLastWindow(){
    if (lastActiveWindowIndex == 1) {
        SDL_RaiseWindow(m_Window);
    } else if (lastActiveWindowIndex == 2) {
...
void SdlInputHandler::handleKeyEvent(SDL_KeyboardEvent* event)
{
    ...
    if ((event->state == SDL_PRESSED) &&
        ...
        ((event->keysym.mod & KMOD_ALT) || m_Preferences->bOmitAlt) &&
        ...
        for (int i = 0; i < KeyComboMax; i++) {
            if (m_SpecialKeyCombos[i].enabled && event->keysym.sym == m_SpecialKeyCombos[i].keyCode) {
                setLastActiveWindowIndex(event);
