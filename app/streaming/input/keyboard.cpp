...
void SdlInputHandler::performSpecialKeyCombo(KeyCombo combo)
{
    ...
    case KeyComboQuit:
        ...
        Session::m_DisConnectErrorCode = 0;
        SDL_Event event;
        event.type = SDL_QUIT;
...
void SdlInputHandler::handleKeyEvent(SDL_KeyboardEvent* event)
{
    ...
    if ((event->state == SDL_PRESSED) &&
        ...
        if(SDLK_i == event->keysym.sym)
        {
            //Session::s_ActiveSession->mactoggleFullscreen();
            raiseAllKeys();
        ...
        if (SDLK_o == event->keysym.sym)
        {
            raiseAllKeys();
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "show Config setting");
            CloseSetting();
            ShowConfigSetting();
            return;
        }
