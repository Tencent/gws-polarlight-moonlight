...
int FFmpegVideoDecoder::getDecoderCapabilities()
{
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "capabilities =  %d",  capabilities);
    return capabilities;
}
...
{
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::reset 1 = %p", m_DecoderThread);
    ...
    if (m_DecoderThread != nullptr) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::SDL_WaitThread begin %d", m_streamId);
        SDL_AtomicSet(&m_DecoderThreadShouldQuit, 1);
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::SDL_WaitThread1 %d", m_streamId);

        LiWakeWaitForVideoFrame(m_streamId);                //自己通知自己断开解码器线程 TODO

        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::SDL_WaitThread2 %d", m_streamId);
        Wakeup();
        SDL_WaitThread(m_DecoderThread, NULL);

        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::SDL_WaitThread3");

        SDL_AtomicSet(&m_DecoderThreadShouldQuit, 0);

        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::m_DecoderThread end");
        m_DecoderThread = nullptr;
    }

    m_FramesIn = m_FramesOut = 0;
    m_FrameInfoQueue.clear();
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "begin delete m_Pacer");
    delete m_Pacer;
    m_Pacer = nullptr;
...
void FFmpegVideoDecoder::stringifyVideoStats(VIDEO_STATS& stats, char* output)
{
    ...
    case VIDEO_FORMAT_H265_MAIN10:
        if (LiGetCurrentHostDisplayHdrMode(m_streamId)) {
...
bool FFmpegVideoDecoder::initialize(PDECODER_PARAMETERS params)
{
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "FFmpegVideoDecoder::initialize");
    ...
    // No decoder worked
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "No decoder worked");
    return false;
}
...
{    
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "decoderThreadProc begin %d", m_streamId);
    while (!SDL_AtomicGet(&m_DecoderThreadShouldQuit)) {
        {//Pause method
            ...
            if (m_pauseFlag) {
                ...
                m_cvPause.wait(lock, [this] {
                    return !m_pauseFlag || SDL_AtomicGet(&m_DecoderThreadShouldQuit); });
            }
        }
        if (m_FramesIn == m_FramesOut) {
            ...
            if (!LiWaitForNextVideoFrame(m_streamId,&handle, &du))      //等待decodeUnitQueue 队列数据帧
            {
                // This might be a signal from the main thread to exit
                //SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,"FFmpegVideoDecoder::decoderThreadProc LiWaitForNextVideoFrame" );
                continue ;
            }

            LiCompleteVideoFrame(m_streamId,handle, submitDecodeUnit(du));
        }
        //SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,  "LiWaitForNextVideoFrame sucess %d,%d",m_FramesIn,m_FramesOut );
        if (m_FramesIn != m_FramesOut) {
            ...
            do {
                ...
                if (err == 0) {
                    ...
                    if (LiPollNextVideoFrame(m_streamId,&handle, &du)) {
                        ...
                        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                                    "Handle EAGAIN on avcodec_send_packet() properly" );
                        LiCompleteVideoFrame(m_streamId,handle, submitDecodeUnit(du));
                    }
                    else {
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "decoderThreadProc End %d" , m_streamId);
}
...
int FFmpegVideoDecoder::submitDecodeUnit(PDECODE_UNIT du)
{
    ...
    if (SDL_TICKS_PASSED(SDL_GetTicks(), m_ActiveWndVideoStats.measurementStartTimestamp + 1000)) {
        ...
        if (Session::get()->getOverlayManager(m_streamId).isOverlayEnabled(Overlay::OverlayDebug)) {
            ...
            stringifyVideoStats(
                lastTwoWndStats,
                Session::get()->getOverlayManager(m_streamId).getOverlayText(Overlay::OverlayDebug));
