...
DxVsyncSource::DxVsyncSource(Pacer *pacer) : m_Pacer(pacer), m_Gdi32Handle(nullptr), m_LastMonitor(nullptr) {
...
bool DxVsyncSource::initialize(SDL_Window *window, int) {
  ...
  if (m_Gdi32Handle == nullptr) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to load gdi32.dll: %d", GetLastError());
    return false;
  }
  ...
  m_D3DKMTWaitForVerticalBlankEvent =
      (PFND3DKMTWAITFORVERTICALBLANKEVENT)GetProcAddress(m_Gdi32Handle, "D3DKMTWaitForVerticalBlankEvent");

  if (m_D3DKMTOpenAdapterFromHdc == nullptr || m_D3DKMTCloseAdapter == nullptr ||
      m_D3DKMTWaitForVerticalBlankEvent == nullptr) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Missing required function in gdi32.dll");
    return false;
  }
...
void DxVsyncSource::waitForVsync() {
  ...
  if (currentMonitor != m_LastMonitor) {
    ...
    if (!GetMonitorInfoA(currentMonitor, &monitorInfo)) {
      SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "GetMonitorInfo() failed: %d", GetLastError());
      return;
    }
    ...
    if (!EnumDisplaySettingsA(monitorInfo.szDevice, ENUM_CURRENT_SETTINGS, &monitorMode)) {
      SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "EnumDisplaySettings() failed: %d", GetLastError());
      return;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Monitor changed: %s %d Hz", monitorInfo.szDevice,
    ...
    if (!openAdapterParams.hDc) {
      SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "CreateDC() failed: %d", GetLastError());
      return;
    }
    ...
    if (status != STATUS_SUCCESS) {
      SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "D3DKMTOpenAdapterFromHdc() failed: %x", status);
      return;
    }
  ...
  if (status != STATUS_SUCCESS) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "D3DKMTWaitForVerticalBlankEvent() failed: %x", status);
    return;
  }
}
