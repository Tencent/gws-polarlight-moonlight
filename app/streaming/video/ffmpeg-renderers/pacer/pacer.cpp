...
Pacer::Pacer(IFFmpegRenderer *renderer, PVIDEO_STATS videoStats)
    : m_RenderThread(nullptr), m_VsyncThread(nullptr), m_Stopping(false), m_VsyncSource(nullptr),
      m_VsyncRenderer(renderer), m_MaxVideoFps(0), m_DisplayFps(0), m_VideoStats(videoStats) {}

Pacer::~Pacer() {
    m_Stopping = true;

    qDebug()<<"Pacer::~Pacer() m_VsyncThread:"<<m_VsyncThread;
    // Stop the V-sync thread
    if (m_VsyncThread != nullptr) {
        qDebug()<<"~Pacer()";
        m_PacingQueueNotEmpty.wakeAll();
        m_VsyncSignalled.wakeAll();         //自己通知自己退出
        SDL_WaitThread(m_VsyncThread, nullptr);
        qDebug()<<"SDL_WaitThread(m_VsyncThread, nullptr) End";
    }

    // Stop V-sync callbacks
    delete m_VsyncSource;
    m_VsyncSource = nullptr;
    qDebug()<<"Pacer::~Pacer() m_RenderThread: "<<m_RenderThread;
    // Stop the render thread
    if (m_RenderThread != nullptr) {
        m_RenderQueueNotEmpty.wakeAll();        //自己通知自己退出
        SDL_WaitThread(m_RenderThread, nullptr);
        qDebug()<<"SDL_WaitThread(m_RenderThread, nullptr) End";
    } else {
        // Notify the renderer that it is being destroyed soon
        // NB: This must happen on the same thread that calls renderFrame().
        qDebug()<<"m_VsyncRenderer->cleanupRenderContext()";
...
int Pacer::vsyncThread(void *context) {
    ...
    qDebug()<<"vsyncThread async = "<<async;
    while (!me->m_Stopping) {
        if (async) {
...
int Pacer::renderThread(void *context) {
    ...
    while (!me->m_Stopping) {
        ...
        me->m_FrameQueueLock.unlock();
        //if we got first frame, fire event to notify UI
        if (!me->m_GotFirstFrame){
            me->m_GotFirstFrame = true;
            SDL_Event event;
            event.type = SDL_RENDER_HAVE_FIRSTFRAME;
            event.window.windowID = SDL_GetWindowID(me->m_Window);
            SDL_PushEvent(&event);
        }
...
bool Pacer::initialize(SDL_Window *window, int maxVideoFps, bool enablePacing) {
    ...
    if (enablePacing) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Frame pacing: target %d Hz with %d FPS stream", m_DisplayFps,
                    m_MaxVideoFps);
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Frame pacing disabled: target %d Hz with %d FPS stream",
                    m_DisplayFps, m_MaxVideoFps);
    }

    if (m_VsyncSource != nullptr) {
        qDebug() << "SDL_CreateThread Pacer::vsyncThread";
        m_VsyncThread = SDL_CreateThread(Pacer::vsyncThread, "PacerVsync", this);
    }

    if (m_VsyncRenderer->isRenderThreadSupported()) {
        qDebug() << "SDL_CreateThread Pacer::renderThread";
...
void Pacer::signalVsync() { m_VsyncSignalled.wakeOne(); }
