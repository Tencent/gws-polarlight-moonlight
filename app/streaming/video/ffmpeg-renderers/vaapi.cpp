...
VAAPIRenderer::VAAPIRenderer(int decoderSelectionPass)
    : m_DecoderSelectionPass(decoderSelectionPass), m_HwContext(nullptr), m_BlacklistedForDirectRendering(false),
...
VADisplay VAAPIRenderer::openDisplay(SDL_Window *window) {
    SDL_SysWMinfo info;
    VADisplay display;
    ...
    if (info.subsystem == SDL_SYSWM_X11) {
        ...
        if (display == nullptr) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to open X11 display for VAAPI");
            return nullptr;
        }
#else
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Gleam not compiled with VAAPI X11 support!");
        return nullptr;
#endif
        ...
        if (display == nullptr) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to open Wayland display for VAAPI");
            return nullptr;
        }
#else
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Gleam not compiled with VAAPI Wayland support!");
        return nullptr;
#endif
    }
        ...
        if (display == nullptr) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to open DRM display for VAAPI");
            return nullptr;
        }
    }
#endif
    else {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unsupported VAAPI rendering subsystem: %d", info.subsystem);
        return nullptr;
    }
...
bool VAAPIRenderer::initialize(PDECODER_PARAMETERS params) {
    int err;
    ...
    if (!m_HwContext) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to allocate VAAPI context");
        return false;
    }
    ...
    for (;;) {
        ...
        if (status != VA_STATUS_SUCCESS && qEnvironmentVariableIsEmpty("LIBVA_DRIVER_NAME")) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Trying fallback VAAPI driver names");
        ...
        if (qEnvironmentVariableIsEmpty("LIBVA_DRIVERS_PATH")) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Trying fallback VAAPI driver paths");
    ...
    if (status != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to initialize VAAPI: %d", status);
        return false;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Initialized VAAPI %d.%d", major, minor);

    const char *vendorString = vaQueryVendorString(vaDeviceContext->display);
    QString vendorStr(vendorString);
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Driver: %s", vendorString ? vendorString : "<unknown>");
    ...
    if (m_DecoderSelectionPass == 0 && (major == 0 || m_HasRfiLatencyBug) && qgetenv("FORCE_VAAPI") != "1" &&
    ...
    if (err < 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to initialize VAAPI context: %d", err);
        return false;
    }
    ...
    if (vaDeviceContext->driver_quirks & AV_VAAPI_DRIVER_QUIRK_SURFACE_ATTRIBUTES) {
        ...
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Avoiding VDPAU wrapper for VAAPI decoding");
        return false;
    }
    ...
    if (m_OverlayMutex == nullptr) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create overlay mutex");
        return false;
    }
    ...
    if (formatCount != 0) {
        ...
        if (status == VA_STATUS_SUCCESS) {
            for (unsigned int i = 0; i < formatCount; i++) {
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Selected overlay subpicture format: %c%c%c%c8888",
                            (m_OverlayFormat.fourcc >> 0) & 0xff, (m_OverlayFormat.fourcc >> 8) & 0xff,
                            (m_OverlayFormat.fourcc >> 16) & 0xff, (m_OverlayFormat.fourcc >> 24) & 0xff);
                break;
            }
        } else {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaQuerySubpictureFormats() failed: %d", status);
        }
...
bool VAAPIRenderer::prepareDecoderContext(AVCodecContext *context, AVDictionary **) {
    context->hw_device_ctx = av_buffer_ref(m_HwContext);

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using VAAPI accelerated renderer on %s", SDL_GetCurrentVideoDriver());

    return true;
}

bool VAAPIRenderer::needsTestFrame() {
    // We need a test frame to see if this VAAPI driver
    // supports the profile used for streaming
    return true;
}

bool VAAPIRenderer::isDirectRenderingSupported() {
    if (qgetenv("VAAPI_FORCE_DIRECT") == "1") {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using direct rendering due to environment variable");
        return true;
    } else if (qgetenv("VAAPI_FORCE_INDIRECT") == "1") {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using indirect rendering due to environment variable");
        return false;
    }
    ...
    if (m_WindowSystem != SDL_SYSWM_X11 || m_BlacklistedForDirectRendering) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using indirect rendering due to WM or blacklist");
        return false;
    } else if (m_VideoFormat & VIDEO_FORMAT_MASK_10BIT) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using indirect rendering for 10-bit video");
        return false;
    }
    ...
    if (status == VA_STATUS_SUCCESS) {
        for (int i = 0; i < entrypointCount; i++) {
            ...
            if (entrypoints[i] == VAEntrypointVideoProc) {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using direct rendering with VAEntrypointVideoProc");
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Using indirect rendering due to lack of VAEntrypointVideoProc");
    return false;
}
...
int VAAPIRenderer::getDecoderColorspace() {
    ...
    // Intel-vaapi-driver defaults to Rec 601 -
    // https://github.com/intel/intel-vaapi-driver/blob/021bcb79d1bd873bbd9fbca55f40320344bab866/src/i965_output_dri.c#L186
...
void VAAPIRenderer::notifyOverlayUpdated(Overlay::OverlayType type) {
    ...
    if (oldSubpictureId != 0) {
        ...
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaDestroySubpicture() failed: %d", status);
        }
    }
    if (oldImageId != 0) {
        status = vaDestroyImage(vaDeviceContext->display, oldImageId);
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaDestroyImage() failed: %d", status);
        }
    }
    ...
    if (newSurface != nullptr) {
        ...
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaCreateImage() failed: %d", status);
            SDL_FreeSurface(newSurface);
            return;
        }
        ...
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaMapBuffer() failed: %d", status);
            SDL_FreeSurface(newSurface);
        ...
        SDL_ConvertPixels(newSurface->w, newSurface->h, newSurface->format->format, newSurface->pixels,
                          newSurface->pitch, m_OverlaySdlPixelFormat, imagePixels, (int)newImage.pitches[0]);

        status = vaUnmapBuffer(vaDeviceContext->display, newImage.buf);
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaUnmapBuffer() failed: %d", status);
            SDL_FreeSurface(newSurface);
            vaDestroyImage(vaDeviceContext->display, newImage.image_id);
            return;
        }
        ...
        if (status != VA_STATUS_SUCCESS) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaCreateSubpicture() failed: %d", status);
            vaDestroyImage(vaDeviceContext->display, newImage.image_id);
            return;
        }
...
void VAAPIRenderer::renderFrame(AVFrame *frame) {
    ...
    if (m_WindowSystem == SDL_SYSWM_X11) {
        ...
        for (int type = 0; type < Overlay::OverlayMax; type++) {
            ...
            status =
                vaAssociateSubpicture(vaDeviceContext->display, m_OverlaySubpicture[type], &surface, 1, 0, 0,
                                      m_OverlayImage[type].width, m_OverlayImage[type].height, m_OverlayRect[type].x,
                                      m_OverlayRect[type].y, m_OverlayRect[type].w, m_OverlayRect[type].h, 0);
            if (status == VA_STATUS_SUCCESS) {
                ...
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaAssociateSubpicture() failed: %d", status);
            }
        }
        ...
        vaPutSurface(vaDeviceContext->display, surface, m_XWindow, 0, 0, m_VideoWidth, m_VideoHeight, dst.x, dst.y,
                     dst.w, dst.h, NULL, 0, flags);
        ...
        for (int type = 0; type < Overlay::OverlayMax; type++) {
            ...
            if (status != VA_STATUS_SUCCESS) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaDeassociateSubpicture() failed: %d", status);
            }
...
bool VAAPIRenderer::canExportSurfaceHandle(int layerTypeFlag) {
    AVHWDeviceContext *deviceContext = (AVHWDeviceContext *)m_HwContext->data;
    AVVAAPIDeviceContext *vaDeviceContext = (AVVAAPIDeviceContext *)deviceContext->hwctx;
    ...
    attrs[attributeCount].flags = VA_SURFACE_ATTRIB_SETTABLE;
    attrs[attributeCount].value.type = VAGenericValueTypeInteger;
    attrs[attributeCount].value.value.i = (m_VideoFormat & VIDEO_FORMAT_MASK_10BIT) ? VA_FOURCC_P010 : VA_FOURCC_NV12;
    attributeCount++;

    st = vaCreateSurfaces(vaDeviceContext->display,
                          (m_VideoFormat & VIDEO_FORMAT_MASK_10BIT) ? VA_RT_FORMAT_YUV420_10 : VA_RT_FORMAT_YUV420,
                          1280, 720, &surfaceId, 1, attrs, attributeCount);
    if (st != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaCreateSurfaces() failed: %d", st);
        return false;
    }

    st = vaExportSurfaceHandle(vaDeviceContext->display, surfaceId, VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
                               VA_EXPORT_SURFACE_READ_ONLY | layerTypeFlag, &descriptor);

    vaDestroySurfaces(vaDeviceContext->display, &surfaceId, 1);

    if (st != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaExportSurfaceHandle() failed: %d", st);
        return false;
    }
...
bool VAAPIRenderer::canExportEGL() {
...
AVPixelFormat VAAPIRenderer::getEGLImagePixelFormat() {
    return (m_VideoFormat & VIDEO_FORMAT_MASK_10BIT) ? AV_PIX_FMT_P010 : AV_PIX_FMT_NV12;
}

bool VAAPIRenderer::initializeEGL(EGLDisplay, const EGLExtensions &ext) {
    if (!ext.isSupported("EGL_EXT_image_dma_buf_import")) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "VAAPI-EGL: DMABUF unsupported");
        return false;
    }
    ...
    if (!(m_eglCreateImage && m_eglDestroyImage) && !(m_eglCreateImageKHR && m_eglDestroyImageKHR)) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Missing eglCreateImage()/eglDestroyImage() in EGL driver");
        return false;
    }
...
ssize_t VAAPIRenderer::exportEGLImages(AVFrame *frame, EGLDisplay dpy, EGLImage images[EGL_MAX_PLANES]) {
    ssize_t count = 0;
    ...
    VAStatus st =
        vaExportSurfaceHandle(vaDeviceContext->display, surface_id, VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
                              VA_EXPORT_SURFACE_READ_ONLY | VA_EXPORT_SURFACE_SEPARATE_LAYERS, &m_PrimeDescriptor);
    if (st != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaExportSurfaceHandle failed: %d", st);
        return -1;
    }
    ...
    if (st != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaSyncSurface failed: %d", st);
        goto sync_fail;
    }
    ...
    for (size_t i = 0; i < m_PrimeDescriptor.num_layers; ++i) {
        ...
        if (m_eglCreateImage) {
            images[i] = m_eglCreateImage(dpy, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, nullptr, attribs);
            if (!images[i]) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "eglCreateImage() Failed: %d", eglGetError());
            ...
            images[i] = m_eglCreateImageKHR(dpy, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, nullptr, intAttribs);
            if (!images[i]) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "eglCreateImageKHR() Failed: %d", eglGetError());
                goto create_image_fail;
            }
        }
...
void VAAPIRenderer::freeEGLImages(EGLDisplay dpy, EGLImage images[EGL_MAX_PLANES]) {
    for (size_t i = 0; i < m_PrimeDescriptor.num_layers; ++i) {
...
bool VAAPIRenderer::mapDrmPrimeFrame(AVFrame *frame, AVDRMFrameDescriptor *drmDescriptor) {
    ...
    VAStatus st =
        vaExportSurfaceHandle(vaDeviceContext->display, vaSurfaceId, VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
                              VA_EXPORT_SURFACE_READ_ONLY | VA_EXPORT_SURFACE_COMPOSED_LAYERS, &vaDrmPrimeDescriptor);
    if (st != VA_STATUS_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vaExportSurfaceHandle() failed: %d", st);
        return false;
    }
