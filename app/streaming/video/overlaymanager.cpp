    ...
    m_FontData(Path::readDataFile("ModeSeven.ttf"))
{
    ...
    qreal qtScaleFactor = GetPrimaryDevicePixelRatio();

    m_Overlays[OverlayType::OverlayDebug].fontSize = 20 * qtScaleFactor;
    qDebug() << "*****************************OverlayManager font size*" <<  m_Overlays[OverlayType::OverlayDebug].fontSize;

    m_Overlays[OverlayType::OverlayStatusUpdate].color = {0xCC, 0x00, 0x00, 0xFF};
    m_Overlays[OverlayType::OverlayStatusUpdate].fontSize = 20 * qtScaleFactor;
...
// Function to create a semi-transparent glass-like surface
SDL_Surface* CreateGlassBackground(int width, int height) {
    SDL_Surface* glassSurface = SDL_CreateRGBSurfaceWithFormat(0, width, height, 32, SDL_PIXELFORMAT_ARGB8888);
    SDL_SetSurfaceBlendMode(glassSurface, SDL_BLENDMODE_BLEND);
    // RGBA with alpha for transparency
    SDL_FillRect(glassSurface, NULL, SDL_MapRGBA(glassSurface->format, 200, 200, 200, 128));
    return glassSurface;
}
...
void OverlayManager::notifyOverlayUpdated(OverlayType type)
{
    ...
    if (m_Overlays[type].enabled) {
        ...
        SDL_Surface* surface = TTF_RenderText_Blended_Wrapped(m_Overlays[type].font,
                                                              ...
                                                              m_Overlays[type].color,                                                            wrapLength);
        if(surface) {
            SDL_Surface* glassSurface = CreateGlassBackground(surface->w, surface->h);
            // Blit the text onto the glass background
            SDL_BlitSurface(surface, NULL, glassSurface, NULL);
