...
IAudioRenderer *Session::createAudioRenderer(const POPUS_MULTISTREAM_CONFIGURATION opusConfig) {
    ...
    if (mlAudio == "sdl") {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unknown audio backend: %s", SDL_getenv("ML_AUDIO"));
        return nullptr;
    }
...
int Session::arInit(int /* audioConfiguration */, const POPUS_MULTISTREAM_CONFIGURATION opusConfig,
    ...
    s_ActiveSession->m_OpusDecoder = opus_multistream_decoder_create(
        s_ActiveSession->m_AudioConfig.sampleRate, s_ActiveSession->m_AudioConfig.channelCount,
        s_ActiveSession->m_AudioConfig.streams, s_ActiveSession->m_AudioConfig.coupledStreams,
        s_ActiveSession->m_AudioConfig.mapping, &error);
    if (s_ActiveSession->m_OpusDecoder == NULL) {
        delete s_ActiveSession->m_AudioRenderer;
        s_ActiveSession->m_AudioRenderer = nullptr;
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create decoder: %d", error);
        return -1;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Audio stream has %d channels",
...
void Session::arCleanup() {
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "arCleanup()");
    QMutexLocker locker(&s_ActiveSession->GetSessionMutex());
    delete s_ActiveSession->m_AudioRenderer;
    s_ActiveSession->m_AudioRenderer = nullptr;
...
void Session::arDecodeAndPlaySample(char *sampleData, int sampleLength) {
    int samplesDecoded;
    //SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "arDecodeAndPlaySample");
#ifndef STEAM_LINK
    ...
    if (s_ActiveSession->m_AudioSampleCount == 0) {
        if (SDL_SetThreadPriority(SDL_THREAD_PRIORITY_HIGH) < 0) {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Unable to set audio thread to high priority: %s",
                        SDL_GetError());
        }
    }
#endif
    ...
    if (s_ActiveSession->m_DropAudioEndTime != 0) {
        if (SDL_TICKS_PASSED(SDL_GetTicks(), s_ActiveSession->m_DropAudioEndTime)) {
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Audio drop window has ended");
        } else {
    ...
    }
    {
        //QMutexLocker locker(&s_ActiveSession->GetSessionMutex());
        if (s_ActiveSession->m_AudioRenderer != nullptr) {
            int desiredSize = sizeof(short) * s_ActiveSession->m_AudioConfig.samplesPerFrame *
                s_ActiveSession->m_AudioConfig.channelCount;
            ...
            samplesDecoded = opus_multistream_decode(
                s_ActiveSession->m_OpusDecoder, (unsigned char*)sampleData, sampleLength, (short*)buffer,
                desiredSize / sizeof(short) / s_ActiveSession->m_AudioConfig.channelCount, 0);

            // Update desiredSize with the number of bytes actually populated by the decoding operation
            if (samplesDecoded > 0) {
                SDL_assert(desiredSize >=
                    (int)(sizeof(short) * samplesDecoded * s_ActiveSession->m_AudioConfig.channelCount));
            ...
            {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Reinitializing audio renderer after failure");
    ...
    // the audio decoder/playback thread is still alive.
    {
        ...
        if (s_ActiveSession->m_AudioRenderer == nullptr && (s_ActiveSession->m_AudioSampleCount % 200) == 0) {
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Audio reinitialization took %d ms - starting drop window",
