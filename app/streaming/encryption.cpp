...
const int PUBLIC_KEY_MAX_LEN = 250;
const int PRIVATE_KEY_MAX_LEN = 887;
#if 0
...
void WriteXml(std::string &UUid, std::vector<std::string> &vUrl) {
    ...
    if (!uuidfile.open(QIODevice::ReadWrite)) {
        qDebug() << "uuidfile open the file for writing.";
        return;
    }
...
bool tesRSAKey(std::string strKey[2], QFile& public_file, QFile& privete_file) {
    ...
    if (!bpubAesFlag || !bpriAesFlag) {
        qDebug() << "public private key AesKey dencrypt error!";
        return "";
    }
    strKey[0] = string(vecPub.data(), vecPub.size());
    strKey[1] = string(vecPri.data(), vecPri.size());
    // qDebug() << "aeskey sucess public :"<<strKey[0].c_str() ;
    // qDebug() << "aeskey  sucess private :"<<strKey[1].c_str() ;
    // test
...
std::string generateRSAKey(std::string strKey[2], std::vector<std::string> &vUrl, bool &bNew) {
    ...
    if (bNew) {
        ...
        if (public_file.exists()) {
            public_file.remove();
            qDebug() << "remove public  pem.";
        }
        if (privete_file.exists()) {
            privete_file.remove();
            qDebug() << "remove privete pem.";
        }
    } else if (!bNew) {
    ...
    strKey[1] = pri_key;
    if (!public_file.open(QIODevice::ReadWrite)) {
        qDebug() << "public key file Failed to open the file for writing.";
        return "";
    }
    if (!privete_file.open(QIODevice::ReadWrite)) {
        qDebug() << "private key file Failed to open the file for writing.";
        return "";
    }
    ...
    if (!bpubAesFlag || !bpriAesFlag) {
        qDebug() << "public private key AesKey encrypt error!";
        return "";
    }
    ...
    free(pri_key);
    free(pub_key);
    qDebug() << "create pubu privete pem sucess!";
    return uuidStr;
}
...
bool RsaPubEncrypt(const char *clear_text, int size, const std::string &pub_key,
    ...
    while (pos < size) {
        ...
        if ((size - pos) >= block_len) {
            ret = RSA_public_encrypt(block_len, (const unsigned char *)pstr, (unsigned char *)sub_text, rsa,
                                     RSA_PKCS1_PADDING);
        } else {
            block_len = (size - pos);
            ret = RSA_public_encrypt(block_len, (const unsigned char *)pstr, (unsigned char *)sub_text, rsa,
    ...
    if (!doc.isNull() && jsonerror.error == QJsonParseError::NoError) {
        if (doc.isObject()) {
            ...
            if (object.contains("displayMode")) {
                ...
                // qDebug()<<"szStr = "<<szStr<<" DisplayMode ="<<DisplayMode ;
            }
        }
    }
}
