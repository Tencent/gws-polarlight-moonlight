...
CompatFetcher::CompatFetcher(QObject *parent) : QObject(parent), m_Nam(this) {
    ...
    connect(&m_Nam, &QNetworkAccessManager::finished, this, &CompatFetcher::handleCompatInfoFetched);
}
...
void CompatFetcher::start() {
    ...
    QUrl url("https://Gleam-stream.org/compatibility/" COMPAT_VERSION);
...
bool CompatFetcher::isGfeVersionSupported(QString gfeVersion) {
    ...
    for (int i = 0;; i++) {
        ...
        if (actualVerVal < latestSupportedVal) {
            ...
            qWarning() << "GFE version" << gfeVersion << "is not supported by this version of Gleam";
            return false;
        }
    }
}
