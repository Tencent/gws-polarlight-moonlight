...
static bool inRange(int value, int min, int max) { return value >= min && value <= max; }

// This method returns key's value from QMap where the key is a QString.
// Key matching is case insensitive.
template <typename T> static T mapValue(QMap<QString, T> map, QString key) {
...
GlobalCommandLineParser::ParseResult GlobalCommandLineParser::parse(const QStringList &args) {
  ...
  parser.setApplicationDescription("\n"
                                   "Starts Gleam normally if no arguments are given.\n"
                                   "\n"
                                   "Available actions:\n"
                                   ...
                                   "\n"
                                   "See 'Gleam <action> --help' for help of specific action.");
...
QString QuitCommandLineParser::getHost() const { return m_Host; }
...
void PairCommandLineParser::parse(const QStringList &args) {
  ...
  parser.addValueOption("agentid", "auto pair id");
  parser.addValueOption("token", "pair token");
  parser.addValueOption("sjson", "https post computer infor  ");
  parser.addValueOption("arch", "https post computer infor  ");
  parser.addValueOption("phost", "map connect pair request url ");
  parser.addValueOption("fingerprint", "fingerprint");
  /*parser.addValueOption("tport1", "map connect pair tcp port");
  parser.addValueOption("tport2", "map connect pair tcp port");
  parser.addValueOption("tport3", "map connect pair tcp port");
  parser.addValueOption("uport1", "map connect pair udp port");
  parser.addValueOption("uport2", "map connect pair udp port");
  parser.addValueOption("uport3", "map connect pair udp port");
  parser.addValueOption("uport4", "map connect pair udp port");
  parser.addValueOption("uport5", "map connect pair udp port");
  parser.addValueOption("uport6", "map connect pair udp port");*/
  ...
  m_InstanceId = parser.value("agentid");
  m_Token = parser.value("token");
  m_HttpsJson = parser.value("sjson");
  m_Arch = parser.value("arch");
  m_PairHost = parser.value("phost");
  if (!m_PredefinedPin.isEmpty() && m_PredefinedPin.length() != 4) {
    parser.showError("PIN must be 4 digits");
  }
}

QString PairCommandLineParser::getHost() const { return m_Host; }

QString PairCommandLineParser::getPredefinedPin() const { return m_PredefinedPin; }

QString PairCommandLineParser::getInstanceId() const { return m_InstanceId; }
QString PairCommandLineParser::getToken() const { return m_Token; }
QString PairCommandLineParser::getHttpsJson() const { return m_HttpsJson; }
QString PairCommandLineParser::getArch() const { return m_Arch; }
QString PairCommandLineParser::getPairHost() const { return m_PairHost; }
...
void StreamCommandLineParser::parse(const QStringList &args, StreamingPreferences *preferences) {
  ...
  parser.addToggleOption("mute-on-focus-loss", "mute audio when Gleam window loses focus");
  ...
  preferences->absoluteTouchMode =
      !parser.getToggleOptionValue("touchscreen-trackpad", !preferences->absoluteTouchMode);
  ...
  preferences->reverseScrollDirection =
      parser.getToggleOptionValue("reverse-scroll-direction", preferences->reverseScrollDirection);
  ...
  if (parser.isSet("capture-system-keys")) {
    preferences->captureSysKeysMode =
        mapValue(m_CaptureSysKeysModeMap, parser.getChoiceOptionValue("capture-system-keys"));
  }
...
QString StreamCommandLineParser::getHost() const { return m_Host; }

QString StreamCommandLineParser::getAppName() const { return m_AppName; }
...
QString ListCommandLineParser::getHost() const { return m_Host; }

bool ListCommandLineParser::isPrintCSV() const { return m_PrintCSV; }

bool ListCommandLineParser::isVerbose() const { return m_Verbose; }
