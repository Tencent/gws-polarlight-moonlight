...
uint16_t NvHTTP::httpPort() { return m_BaseUrlHttp.port(); }

uint16_t NvHTTP::httpsPort() { return m_BaseUrlHttps.port(); }
...
void NvHTTP::startApp(QString verb, bool isGfe, int appId, PSTREAM_CONFIGURATION streamConfig, bool sops,
  ...
  QString response = openConnectionToString(
      ...
      "appid=" + QString::number(appId) + "&clientuuid=" + (UUid.c_str()) +
          "&mode=" + QString::number(streamConfig->width) + "x" + QString::number(streamConfig->height) + "x" +
  ...
  // qInfo() << "decrypt_text:" << decrypt_text;
}
    ...
    if (xmlReader.name() == QString("root")) {
      ...
      if (statusCode == 200) {
        ...
        if (statusCode == -1 && statusMessage == "Invalid") {
          ...
          statusCode = 418;
          statusMessage =
              tr("Missing audio capture device. Reinstalling GeForce Experience should resolve this error.");
        }
...
QNetworkReply *NvHTTP::waitForReply(QNetworkRequest& request, QUrl& url, int timeoutMs, NvLogLevel logLevel) {
...
QNetworkReply *NvHTTP::openConnection(QUrl baseUrl, QString command, QString arguments, int timeoutMs,
                                      NvLogLevel logLevel) {
  // Port must be set
...
std::string NvReqHTTPS::RequestByToken(QString &baseUrl, QString token, QJsonDocument body, int timeoutMs, int &nRet) {
  nRet = 0;
  std::string OutStr;
  QUrl url(baseUrl);
  QNetworkRequest request(url);
...
QString NvHTTP::SendMsg(QString &msg) {
  QString szStr = openConnectionToString(m_BaseUrlHttps, "sendmsg", msg, QUIT_TIMEOUT_MS);

  qInfo() << "SendMsg response:" << szStr;

  // Throws if the request failed
  verifyResponseStatus(szStr);
  // Populate the HTTPS port
  QString szRecv = getXmlString(szStr, "gleamRet");
  return szRecv;
}
