...
#define POLLS_PER_APPLIST_FETCH 10
  ...
  public:
    PcMonitorThread(NvComputer *computer) : m_Computer(computer) {
        ...
        while (!isInterruptionRequested()) {
            ...
            for (int i = 0; i < (wasOnline ? TRIES_BEFORE_OFFLINING : 1) && !online; i++) {
                for (auto &address : m_Computer->uniqueAddresses()) {
                    ...
                    QString qstr("Checking target computer status (");
                    qstr += QString::number(iLoopCnt) + ")";
                    UIStateMachine::I().push(s_iOpId, MsgCode::NO_CODE, MsgType::Information, qstr.toStdString());
...
#ifndef APP_LIST_REFRESH
            // Grab the applist if it's empty or it's been long enough that we need to refresh
            pollsSinceLastAppListFetch++;
            if (m_Computer->state == NvComputer::CS_ONLINE && m_Computer->pairState == NvComputer::PS_PAIRED &&
...
ComputerManager::ComputerManager(QObject *parent)
    : QObject(parent), m_PollingRef(0), m_MdnsBrowser(nullptr), m_CompatFetcher(nullptr), m_NeedsDelayedFlush(false) {
    QSettings settings;
...
bool ComputerManager::matchComputerWithName(NvComputer* computer, QString name) const
{
...
//Only seek the computer passed in, will be called from new Core engine (Shawn 7/16/2024)
void ComputerManager::startPolling(QString computerNameOrIp) {
    QWriteLocker lock(&m_Lock);
    qInfo() << "ComputerManager::startPolling ";
    if (++m_PollingRef > 1) {
        return;
    }

    StreamingPreferences prefs;
    qDebug() << "prefs.enableMdns = " << prefs.enableMdns;
    if (prefs.enableMdns) {
        ...
        connect(m_MdnsBrowser, &QMdnsEngine::Browser::serviceAdded, this, [this](const QMdnsEngine::Service& service) {
            ...
            connect(pendingComputer, &MdnsPendingComputer::resolvedHost, this,
                &ComputerManager::handleMdnsServiceResolved);
    ...
    while (i.hasNext()) {
        i.next();
        auto* pComputer = i.value();
        if (matchComputerWithName(pComputer, computerNameOrIp)) {
            startPollingComputer(pComputer);
            break;
        }
    }
}
...
void ComputerManager::startPollingComputer(NvComputer *computer) {
    ...
    if (!pollingEntry->isActive()) {
        ...
        connect(thread, &PcMonitorThread::computerStateChanged, this, &ComputerManager::handleComputerStateChanged);
        pollingEntry->setActiveThread(thread);
        thread->start();
    }
}

void ComputerManager::handleMdnsServiceResolved(MdnsPendingComputer *computer, QVector<QHostAddress> &addresses) {
    ...
    if (!added) {
        ...
        for (const QHostAddress &address : addresses) {
            if (address.protocol() == QAbstractSocket::IPv6Protocol) {
                ...
                if (address.isInSubnet(QHostAddress("fe80::"), 10) || address.isInSubnet(QHostAddress("fec0::"), 10) ||
...
class DeferredHostDeletionTask : public QRunnable {
  public:
    DeferredHostDeletionTask(ComputerManager *cm, NvComputer *computer) : m_Computer(computer), m_ComputerManager(cm) {}
...
class RequstGwsPairingTask : public QObject, public QRunnable {
    Q_OBJECT

  public:
    RequstGwsPairingTask(ComputerManager *computerManager, NvComputer *computer, QString pin, QString token,
                         QString instanceid, QString phost)
        : m_Computer(computer), m_Pin(pin), m_Token(token),
          m_InstanceId(instanceid), m_phost(phost) {
        connect(this, &RequstGwsPairingTask::AutoPairError, computerManager, &ComputerManager::AutoPairError);

        connect(computerManager, &ComputerManager::quitAutoHttpsPair, this, &RequstGwsPairingTask::quitAutoHttpsPair);
    }
  protected slots:

    void quitAutoHttpsPair() { m_exit = false; }
  signals:
    void AutoPairError();
  ...
  private:
    void run() {
        ...
        nAutoPairCount = 0;
        while (m_exit) {
            if (nAutoPairCount >= 1) // timeout
            {
                qInfo() << "auto pair time out.........";
                emit AutoPairError();
                m_exit = false;
                return;
            }
            QThread::sleep(1);
            qDebug() << "auto pair begin!";
            std::string pin = m_Pin.toStdString();
            std::string stanceid = m_InstanceId.toStdString(); //"192487919781154816";

            // std::string url = "ces-gws-qa.local.oitces.com";
            std::string url = m_phost.toStdString(); //"portal.oitces.com";
            std::string szUrl = "https://" + url + "/gws/v1/agent/" + stanceid + "/pair?pin=" + pin;
            std::string strtoken = m_Token.toStdString();
            if (stanceid.size() == 0 || strtoken.size() == 0) {
                return;
            }
            NvPairingManager pairingManager(m_Computer);
            int nRet = pairingManager.autoPair(szUrl.c_str(), strtoken.c_str());

            if (0 != nRet) {
                qInfo() << "auto pair  request fail......";
                emit AutoPairError();
                return;
            }
            nAutoPairCount++;
            qDebug() << "auto pair request sucess,wait pair..........";
            QThread::sleep(30);
        }
        qInfo() << "auto pair sucess";
    }
    NvComputer *m_Computer;
    QString m_Pin;
    QString m_Token;
...
class PendingPairingTask : public QObject, public QRunnable {
  ...
  public:
    PendingPairingTask(ComputerManager *computerManager, NvComputer *computer, QString pin, QString token,
        QString instanceid, QString phost): m_ComputerManager(computerManager), m_Computer(computer),
        m_Pin(pin), m_Token(token), m_InstanceId(instanceid), m_phost(phost) {
        connect(this, &PendingPairingTask::pairingCompleted, computerManager, &ComputerManager::pairingCompleted);
    }
    ...
    void run() {
        ...
        if (m_Token.length() > 0 && m_InstanceId.length() > 0) {
            ...
            while (!succ) {
                ...
                if (0 != nRet) {
                    qInfo() << "auto pair request fail, retry later ...";
                    QThread::sleep(1);

                }
                succ = true;
                qInfo() << "auto pair request sucess,wait pair...";
            }
        }

        qInfo() << "start pair to host";
        NvPairingManager pairingManager(m_Computer);

        try {
            NvPairingManager::PairState result =
                pairingManager.pair(m_Computer->appVersion, m_Pin, m_Computer->serverCert);
            switch (result) {
            ...
            case NvPairingManager::PairState::FAILED:
                if (m_Computer->currentGameId != 0) {
                    emit pairingCompleted(
                        m_Computer, tr("You cannot pair while a previous session is still running on the host PC. Quit "
                                       "any running games or reboot the host PC, then try pairing again."));
                } else {
...
void ComputerManager::pairHost(NvComputer *computer, QString pin, QString token, QString instanceid,
                               QString phost) {
    // Punt to a worker thread to avoid stalling the
    // UI while waiting for pairing to complete
    PendingPairingTask *pairing = new PendingPairingTask(this, computer, pin, token, instanceid, phost);
    QThreadPool::globalInstance()->start(pairing);
}
void ComputerManager::RequestGwsPair(NvComputer *computer, QString pin, QString token, QString instanceid,
                                     QString phost) {

    RequstGwsPairingTask *pairing = new RequstGwsPairingTask(this, computer, pin, token, instanceid, phost);
    QThreadPool::globalInstance()->start(pairing);
}

void ComputerManager::AutoPairError()
{
    // PopTips(tr("Timeout when pairing, please reboot and try again. Contact administrator if fail again."));
}
class PendingQuitTask : public QObject, public QRunnable {
    Q_OBJECT

  public:
    PendingQuitTask(ComputerManager *computerManager, NvComputer *computer) : m_Computer(computer) {
        connect(this, &PendingQuitTask::quitAppFailed, computerManager, &ComputerManager::quitAppCompleted);
    }
        ...
        try {
            ...
            if (e.getStatusCode() == 599) {
                ...
                emit quitAppFailed(tr("The running game wasn't started by this PC. "
                                      "You must quit the game on the host PC manually or use the device that "
                                      "originally started the game."));
            } else {
...
void ComputerManager::stopPollingAsync() {
    qInfo()<<"ComputerManager::stopPollingAsync()";
    QWriteLocker lock(&m_Lock);
...
void ComputerManager::addNewHostManually(QString address) {
    QUrl url = QUrl::fromUserInput("gleam://" + address);

    qInfo() << "ComputerManager::addNewHostManually url = " << url;
    if (url.isValid() && !url.host().isEmpty() && url.scheme() == "gleam") {
        // If there wasn't a port specified, use the default
        qInfo() << "url.host() " << url.host();
...
class PendingAddTask : public QObject, public QRunnable {
  ...
  public:
    PendingAddTask(ComputerManager *computerManager, NvAddress address, NvAddress mdnsIpv6Address, bool mdns)
        : m_ComputerManager(computerManager), m_Address(address), m_MdnsIpv6Address(mdnsIpv6Address), m_Mdns(mdns),
          m_AboutToQuit(false) {
        connect(this, &PendingAddTask::computerAddCompleted, computerManager, &ComputerManager::computerAddCompleted);
        connect(this, &PendingAddTask::computerStateChanged, computerManager,
                &ComputerManager::handleComputerStateChanged);
        connect(QCoreApplication::instance(), &QCoreApplication::aboutToQuit, this, &PendingAddTask::handleAboutToQuit);
    }
  ...
  private:
    void handleAboutToQuit() { m_AboutToQuit = true; }
    ...
    QString fetchServerInfo(NvHTTP &http) {
        ...
        try {
            ...
            try {
                ...
                if (e.getError() == QNetworkReply::ServiceUnavailableError || e.getError() == QNetworkReply::TimeoutError) {
                    qWarning() << "Retrying request in 5 seconds after ServiceUnavailableError";
                    UIStateMachine::I().push(s_iOpId, MsgCode::NO_CODE, MsgType::Information, e.getErrorText());
                    QThread::sleep(5);
                    serverInfo = http.getServerInfo(NvHTTP::NVLL_VERBOSE);
            ...
            UIStateMachine::I().push(s_iOpId, MsgCode::Network_Time_Out, MsgType::Information);
            if (!m_Mdns) {
                StreamingPreferences prefs;
                int portTestResult;
                ...
                if (prefs.detectNetworkBlocking) {
                    ...
                    // isn't blocking Gleam, so we can tell the user about it.
                    portTestResult = LiTestClientConnectivity("qt.conntest.Gleam-stream.org", 443,
    ...
    void run() {
        ...
        qInfo() << "Processing new PC at" << m_Address.toString() << "from" << (m_Mdns ? "mDNS" : "user")
                << "with IPv6 address" << m_MdnsIpv6Address.toString();
        ...
        if (m_Mdns) {
            ...
            if (QHostAddress(newComputer->localAddress.address()).protocol() == QAbstractSocket::IPv4Protocol) {
                quint32 addr;
                int err = LiFindExternalAddressIP4("stun.Gleam-stream.org", 3478, &addr);
                if (err == 0) {
        ...
        bool addressIsSiteLocalV4 = hostAddress.isInSubnet(QHostAddress("10.0.0.0"), 8) ||
...

        {
            ...
            if (existingComputer != nullptr) {
                ...
                if (!m_Mdns && addressIsSiteLocalV4 &&
                    newComputer->getActiveAddressReachability() != NvComputer::RI_VPN) {
                    quint32 addr;
                    int err = LiFindExternalAddressIP4("stun.Gleam-stream.org", 3478, &addr);
                    if (err == 0) {
                        newComputer->setRemoteAddress(QHostAddress(qFromBigEndian(addr)));
