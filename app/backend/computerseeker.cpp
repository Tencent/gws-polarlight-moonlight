...
void ComputerSeeker::ResetTo(QString computerName)
{
    ...
    if (m_TimeoutTimer) 
    {
        ...
        disconnect(m_TimeoutTimer, &QTimer::timeout,
            this, &ComputerSeeker::onTimeout);
        m_TimeoutTimer->deleteLater(); // Use deleteLater() to safely delete in the event loop
...
void ComputerSeeker::start(int timeout)
{
    ...
    if (m_OnlySeekMyself)
    {
        m_ComputerManager->startPolling(m_ComputerName);
    }
}
