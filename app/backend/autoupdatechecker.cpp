...
AutoUpdateChecker::AutoUpdateChecker(QObject *parent) : QObject(parent), m_Nam(this) {
    ...
    connect(&m_Nam, &QNetworkAccessManager::finished, this, &AutoUpdateChecker::handleUpdateCheckRequestFinished);

    QString currentVersion(VERSION_STR);
    qDebug() << "Current Gleam version:" << currentVersion;
...
void AutoUpdateChecker::start() {
    ...
    defined(APP_IMAGE) // Only run update checker on platforms without auto-update
    ...
    QUrl url("https://Gleam-stream.org/updates/qt.json");
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        for (QJsonValueRef updateEntry : array) {
            if (updateEntry.isObject()) {
                ...
                if (!updateObj.contains("platform") || !updateObj.contains("arch") || !updateObj.contains("version") ||
                    !updateObj.contains("browser_url")) {
                    qWarning() << "Update manifest entry missing vital field";
                    continue;
                }

                if (!updateObj["platform"].isString() || !updateObj["arch"].isString() ||
                    !updateObj["version"].isString() || !updateObj["browser_url"].isString()) {
                ...
                if (updateObj["arch"] == QSysInfo::buildCpuArchitecture() && updateObj["platform"] == getPlatform()) {
                    ...
                    if (updateObj.contains("kernel_version_at_least") &&
                        updateObj["kernel_version_at_least"].isString()) {
                        ...
                        if (compareVersion(actualVersionQuad, requiredVersionQuad) < 0) {
                            qDebug() << "Skipping manifest entry due to kernel version (" << actualVersion << "<"
                                     << requiredVersion << ")";
                            continue;
                        }
                    }
                    ...
                    qDebug() << "Latest version of Gleam for this platform is:" << latestVersion;
        ...
        qWarning() << "No entry in update manifest found for current platform:" << QSysInfo::buildCpuArchitecture()
                   << getPlatform() << QSysInfo::kernelVersion();
    } else {
