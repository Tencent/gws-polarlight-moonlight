...
#define SER_DUALDISPLAYMODE "dualdisplay"
...
NvComputer::NvComputer(QSettings& settings)
{
    ...
    this->manualAddress = NvAddress(settings.value(SER_MANUALADDR).toString(),
                                    settings.value(SER_MANUALPORT, QVariant(DEFAULT_HTTP_PORT)).toUInt());  //DEFAULT_HTTP_PORT
    ...
    settings.endArray();
    sortAppList();
    this->isDualDisplay = settings.value(SER_DUALDISPLAYMODE).toBool();
...
void NvComputer::serialize(QSettings& settings) const
{
    ...
    settings.setValue(SER_DUALDISPLAYMODE, isDualDisplay);
...
NvComputer::NvComputer(NvHTTP& http, QString serverInfo)
{
    ...
    httpsPort = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_TCP_47984);
    if (httpsPort.isEmpty() || (this->activeHttpsPort = httpsPort.toUShort()) == 0) {
        this->activeHttpsPort = DEFAULT_HTTPS_PORT;     //DEFAULT_HTTPS_PORT
    }
    ...
    qDebug() << "this->currentGameId = " << this->currentGameId <<"this = "<< this;
    ...
    this->isDualDisplay = NvHTTP::getXmlString(serverInfo, "DualDisplayMode") == "1" ?
                              true : false;
...
bool NvComputer::wake() const
{
    ...
    const quint16 STATIC_WOL_PORTS[] = {
        ...
        47009, // Port opened by Gleam Internet Hosting Tool for WoL (non-privileged port)
    };

    // Ports offset by the HTTP base port for hosts using alternate ports
    quint16 DYNAMIC_WOL_PORTS[5] = {
        // 47998, 47999, 48000, 48002, 48010, // Ports opened by GFE
    };

    DYNAMIC_WOL_PORTS[0] = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47998);
    DYNAMIC_WOL_PORTS[1] = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47999);
    DYNAMIC_WOL_PORTS[2] = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48000);
    DYNAMIC_WOL_PORTS[3] = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48002);
    DYNAMIC_WOL_PORTS[4] = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48010);
...
#define ASSIGN_IF_CHANGED_AND_NONNULL(field)  \
    ...
    ASSIGN_IF_CHANGED(isDualDisplay);
