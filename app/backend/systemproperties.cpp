...
SystemProperties::SystemProperties() {
...
#ifdef Q_OS_WIN32
    {
        ...
        auto fnIsWow64Process2 =
            (decltype(IsWow64Process2) *)GetProcAddress(GetModuleHandleA("kernel32.dll"), "IsWow64Process2");
...
class QuerySdlVideoThread : public QThread {
  public:
    QuerySdlVideoThread(SystemProperties *me) : QThread(nullptr), m_Me(me) {}

    void run() override { m_Me->querySdlVideoInfoInternal(); }
...
void SystemProperties::querySdlVideoInfoInternal() {
    ...
    SDL_Window *testWindow =
        SDL_CreateWindow("", 0, 0, 1280, 720, SDL_WINDOW_HIDDEN | StreamUtils::getPlatformWindowFlags());
    if (!testWindow) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Failed to create test window with platform flags: %s",
                    SDL_GetError());
    ...
    Session::getDecoderInfo(testWindow, hasHardwareAcceleration, rendererAlwaysFullScreen, supportsHdr,
                            maximumResolution);
...
class RefreshDisplaysThread : public QThread {
  public:
    RefreshDisplaysThread(SystemProperties *me) : QThread(nullptr), m_Me(me) {}

    void run() override { m_Me->refreshDisplaysInternal(); }
    ...
    for (int displayIndex = 0; displayIndex < SDL_GetNumVideoDisplays(); displayIndex++) {
        ...
        if (StreamUtils::getNativeDesktopMode(displayIndex, &desktopMode)) {
            if (desktopMode.w <= 8192 && desktopMode.h <= 8192) {
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Skipping resolution over 8K: %dx%d", desktopMode.w,
                            desktopMode.h);
            }
...
int SystemProperties::getMaxFps(std::set<int> &monitorFps)
{
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_InitSubSystem(SDL_INIT_VIDEO) failed: %s", SDL_GetError());
        return 0;
    ...
    for (int displayIndex = 0; displayIndex < SDL_GetNumVideoDisplays(); displayIndex++) {
        ...
        if (StreamUtils::getNativeDesktopMode(displayIndex, &desktopMode)) {
            ...
            if (bestMode.refresh_rate > 62)
            {
                monitorFps.insert(bestMode.refresh_rate);
            }
        }
    }
