...
#define INIT_PAIR_TIMEOUT_MS 60000

NvPairingManager::NvPairingManager(NvComputer *computer) : m_Http(computer) {
...
QByteArray NvPairingManager::decrypt(const QByteArray &ciphertext, const QByteArray &key) {
    ...
    EVP_DecryptUpdate(cipher, reinterpret_cast<unsigned char *>(plaintext.data()), &plaintextLen,
                      reinterpret_cast<const unsigned char *>(ciphertext.data()), ciphertext.length());
...
bool NvPairingManager::verifySignature(const QByteArray &data, const QByteArray &signature,
    ...
    int result = EVP_DigestVerifyFinal(mdctx, reinterpret_cast<unsigned char *>(const_cast<char *>(signature.data())),
                                       signature.length());
...
QByteArray NvPairingManager::saltPin(const QByteArray &salt, QString pin) {
    return QByteArray().append(salt).append(pin.toLatin1());
}

NvPairingManager::PairState NvPairingManager::pair(QString appVersion, QString pin, QSslCertificate &serverCert) {
    ...
    QString getCert =
        m_Http.openConnectionToString(m_Http.m_BaseUrlHttp, "pair",
                                      "devicename=roth&updateState=1&phrase=getservercert&salt=" + salt.toHex() +
                                          "&clientcert=" + IdentityManager::get()->getCertificate().toHex(),
                                      INIT_PAIR_TIMEOUT_MS);
    ...
    QString challengeXml = m_Http.openConnectionToString(
        m_Http.m_BaseUrlHttp, "pair", "devicename=roth&updateState=1&clientchallenge=" + encryptedChallenge.toHex(),
        REQUEST_TIMEOUT_MS);
    ...
    QString respXml = m_Http.openConnectionToString(m_Http.m_BaseUrlHttp, "pair",
    ...
    if (!verifySignature(serverSecret, serverSignature, serverCertStr)) {
    ...
    QString secretRespXml = m_Http.openConnectionToString(
        m_Http.m_BaseUrlHttp, "pair",
        "devicename=roth&updateState=1&clientpairingsecret=" + clientPairingSecret.toHex(), REQUEST_TIMEOUT_MS);
    ...
    QString pairChallengeXml = m_Http.openConnectionToString(
        m_Http.m_BaseUrlHttps, "pair", "devicename=roth&updateState=1&phrase=pairchallenge", REQUEST_TIMEOUT_MS);
...
int NvPairingManager::autoPair(QString url, QString token) {
    QJsonDocument body;
    int nRet = 0;
    m_Http.GwsHttpsPost(url, token, body, nRet);
    return nRet;
}

int NvPairingManager::sendSelfComputerInforToGws(QString url, QString token, QJsonDocument body) {
    int nRet = 0;
    m_Http.GwsHttpsPost(url, token, body, nRet);
    return nRet;
}
