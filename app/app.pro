QT += core quick network quickcontrols2 quickwidgets svg webenginewidgets widgets core-private
CONFIG += c++20
...
win32 {
    ...
    contains(QT_ARCH, arm64) {
     ...
     INCLUDEPATH += $$PWD/../lib/windows/include
...
macx {
    ...
    QMAKE_CXXFLAGS += -Wno-unguarded-availability-new
...
macx {
    LIBS += -lssl.3 -lcrypto.3 -lavcodec.61 -lavutil.59 -lopus -framework SDL2 -framework SDL2_ttf
...
SOURCES += \
    Core/IpFetcher.cpp \
    Core/displayInfo.cpp \
    Core/helpWebView.cpp \
    Core/potalInteractionManager.cpp \
    Core/settingWebView.cpp \
    Core/transparentWebView.cpp \
    Core/uiStateMachine.cpp \
    ...
    streaming/encryption.cpp \
    streaming/gleamapi.cpp \
...
HEADERS += \
    ...
    Core/displayInfo.h \
    Core/helpWebView.h \
    Core/httpmessagetask.h \
    Core/potalInteractionManager.h \
    Core/settingWebView.h \
    Core/transparentWebView.h \
    Core/uiStateMachine.h \
    EventReporter.h \
    backend/nvaddress.h \
    backend/nvapp.h \
    cli/pair.h \
    ...
    soloInstance.h \
    streaming/CountdownWindow.h \
    streaming/connection.h \
    streaming/encryption.h \
    streaming/eventManager.h \
    streaming/gleamapi.h \
    streaming/share_data.h \
    utils.h \
...
win32:CONFIG(release, debug|release): POST_TARGETDEPS += $$OUT_PWD/../moonlight-common-c/release/moonlight-common-c.lib
else:win32:CONFIG(debug, debug|release): POST_TARGETDEPS += $$OUT_PWD/../moonlight-common-c/debug/moonlight-common-c.lib
else:unix: POST_TARGETDEPS += $$OUT_PWD/../moonlight-common-c/libmoonlight-common-c.a

INCLUDEPATH += $$PWD/../moonlight-common-c/moonlight-common-c/src_cpp
DEPENDPATH += $$PWD/../moonlight-common-c/moonlight-common-c/src_cpp
...
unix:!macx: {
    ...
    desktop.files = deploy/linux/com.Gleam_stream.Gleam.desktop
    desktop.path = $$PREFIX/$$DATADIR/applications/

    icons.files = res/Gleam.svg
    icons.path = $$PREFIX/$$DATADIR/icons/hicolor/scalable/apps/

    appstream.files = deploy/linux/com.Gleam_stream.Gleam.appdata.xml
...
win32 {
    RC_ICONS = Gleam.ico
    QMAKE_TARGET_COMPANY = Gleam Game Streaming Project
    QMAKE_TARGET_DESCRIPTION = Gleam Game Streaming Client
    QMAKE_TARGET_PRODUCT = Gleam

    CONFIG -= embed_manifest_exe
    QMAKE_LFLAGS += /MANIFEST:embed /MANIFESTINPUT:$${PWD}/Gleam.exe.manifest
}
macx {
    # Create Info.plist in object dir with the correct version string
    system(cp $$PWD/Info.plist $$OUT_PWD/Info.plist)
    system(sed -i -e 's/VERSION/$$cat(../version)/g' $$OUT_PWD/Info.plist)

    QMAKE_INFO_PLIST = $$OUT_PWD/Info.plist

    APP_BUNDLE_RESOURCES.files = Gleam.icns
...
VERSION = "$$cat(../version)"
DEFINES += VERSION_STR=\\\"$$cat(../version)\\\"

DISTFILES += \
    about.png \
    webpages/config.html \
    webpages/vue.js
