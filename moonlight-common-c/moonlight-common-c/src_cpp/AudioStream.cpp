...
static void AudioPingThreadProc(void *context) {
  ...
  while (!PltIsThreadInterrupted(&udpPingThread)) {
    if (AudioPingPayload.payload[0] != 0) {
      ...
      sendto(rtpSocket, (char *)&AudioPingPayload, sizeof(AudioPingPayload), 0, (struct sockaddr *)&saddr,
             RemoteAddrLen);
    } else {
...
static void decodeInputData(PQUEUED_AUDIO_PACKET packet, char *AesKey) {
  ...
  if (AudioEncryptionEnabled) {
    ...
    if (AesKey) {
      if (!PltDecryptMessage(audioDecryptionCtx, ALGORITHM_AES_CBC, CIPHER_FLAG_RESET_IV | CIPHER_FLAG_FINISH,
                             (unsigned char *)AesKey, 16, iv, sizeof(iv), NULL, 0, (unsigned char *)(rtp + 1),
                             dataLength, decryptedOpusData, &dataLength)) {
      ...
      if (!PltDecryptMessage(audioDecryptionCtx, ALGORITHM_AES_CBC, CIPHER_FLAG_RESET_IV | CIPHER_FLAG_FINISH,
                             ...
                             iv, sizeof(iv), NULL, 0, (unsigned char *)(rtp + 1), dataLength, decryptedOpusData,
                             &dataLength)) {
        // Limelog("Failed to decrypt audio packet (sequence number: %u)\n", rtp->sequenceNumber);
        // LC_ASSERT(false);
        return;
  ...
  while (!PltIsThreadInterrupted(&receiveThread)) {
    ...
    if (RTPQ_HANDLE_NOW(queueStatus)) {
      if ((AudioCallbacks.capabilities & CAPABILITY_DIRECT_SUBMIT) == 0) {
        ...
        if (g_Encrypted_AES_Key.size() > 0) {
          decodeInputData(packet, g_Encrypted_AES_Key.data());
        } else {
          decodeInputData(packet, NULL);
        }
      }
    } else {
      ...
      if (RTPQ_PACKET_READY(queueStatus)) {
        ...
        while ((queuedPacket = (PQUEUED_AUDIO_PACKET)RtpaGetQueuedPacket(
                    &rtpAudioQueue, sizeof(QUEUED_AUDIO_PACKET_HEADER), &length)) != NULL) {
...
int LiGetPendingAudioFrames(void) { return LbqGetItemCount(&packetQueue); }

int LiGetPendingAudioDuration(void) { return LiGetPendingAudioFrames() * AudioPacketDuration; }
