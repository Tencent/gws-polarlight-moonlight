...
static POPTION_ITEM createOptionItem(char *option, char *content) {
  POPTION_ITEM item = (POPTION_ITEM)malloc(sizeof(*item));
  if (item == NULL) {
    return NULL;
  }
...
static bool initializeRtspRequest(PRTSP_MESSAGE msg, char *command, char *target) {
  ...
  createRtspRequest(msg, NULL, 0, command, target, (char *)"RTSP/1.0", 0, NULL, NULL, 0);

  sprintf(sequenceNumberStr, "%d", currentSeqNumber++);
  sprintf(clientVersionStr, "%d", rtspClientVersion);
  if (!addOption(msg, (char *)"CSeq", sequenceNumberStr) ||
      !addOption(msg, (char *)"X-GS-ClientVersion", clientVersionStr) ||
      (!useEnet && !addOption(msg, (char *)"Host", urlAddr))) {
    freeMessage(msg);
    return false;
  }
...
static bool transactRtspMessageEnet(PRTSP_MESSAGE request, PRTSP_MESSAGE response, bool expectingPayload, int *error) {
  ...
  *error = -1;
  ...
  // Wait for a reply
  if (serviceEnetHost(client, &event, RTSP_RECEIVE_TIMEOUT_SEC * 1000) <= 0 || event.type != ENET_EVENT_TYPE_RECEIVE) {
    Limelog("Failed to receive RTSP reply\n");
    goto Exit;
  }

  responseBuffer = (char *)malloc(event.packet->dataLength);
  ...
  if (expectingPayload) {
    ...
    responseBuffer = (char *)extendBuffer(responseBuffer, event.packet->dataLength + offset);
    if (responseBuffer == NULL) {
...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int *error) {
  ...
  *error = -1;
  ...
  do {
    ...
    Limelog("transactRtspMessageTcp...connectRetries = %d", connectRetries);
  ...
  for (;;) {
    ...
    if (offset >= responseBufferSize) {
      ...
      responseBuffer = (char *)extendBuffer(responseBuffer, responseBufferSize);
      if (responseBuffer == NULL) {
        Limelog("Failed to allocate RTSP response buffer\n");
        goto Exit;
      }
...
static bool requestOptions(PRTSP_MESSAGE response, int *error) {
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(&request, (char *)"OPTIONS", rtspTargetUrl);
  if (ret) {
...
static bool requestDescribe(PRTSP_MESSAGE response, int *error) {
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(&request, (char *)"DESCRIBE", rtspTargetUrl);
  if (ret) {
    if (addOption(&request, (char *)"Accept", (char *)"application/sdp") &&
        addOption(&request, (char *)"If-Modified-Since", (char *)"Thu, 01 Jan 1970 00:00:00 GMT")) {
...
static bool setupStream(PRTSP_MESSAGE response, char *target, int *error) {
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(&request, (char *)"SETUP", target);
  if (ret) {
    if (hasSessionId) {
      if (!addOption(&request, (char *)"Session", sessionIdString)) {
        ret = false;
        goto FreeMessage;
      }
    }
    ...
    if (AppVersionQuad[0] >= 6) {
      ...
      // handshake process.
      transportValue = (char *)"unicast;X-GS-ClientPort=50000-50001";
    } else {
      transportValue = (char *)" ";
    }

    if (addOption(&request, (char *)"Transport", transportValue) &&
        addOption(&request, (char *)"If-Modified-Since", (char *)"Thu, 01 Jan 1970 00:00:00 GMT")) {
      ret = transactRtspMessage(&request, response, false, error);
    } else {
      ret = false;
...
static bool playStream(PRTSP_MESSAGE response, char *target, int *error) {
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(&request, (char *)"PLAY", target);
  if (ret != 0) {
    if (addOption(&request, (char *)"Session", sessionIdString)) {
      ret = transactRtspMessage(&request, response, false, error);
    } else {
...
static bool sendVideoAnnounce(PRTSP_MESSAGE response, int *error, const char *target) {
  RTSP_MESSAGE request;
  bool ret;
  int payloadLength;
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(&request, (char *)"ANNOUNCE",
                              APP_VERSION_AT_LEAST(7, 1, 431) ? controlStreamId : (char *)target);
  if (ret) {
    ret = false;

    if (!addOption(&request, (char *)"Session", sessionIdString) ||
        !addOption(&request, (char *)"Content-type", (char *)"application/sdp")) {
      goto FreeMessage;
    }
    ...
    sprintf(payloadLengthStr, (char *)"%d", payloadLength);
    if (!addOption(&request, (char *)"Content-length", payloadLengthStr)) {
      goto FreeMessage;
    }
...
static int parseOpusConfigFromParamString(char *paramStr, int channelCount,
...
static bool parseServerPortFromTransport(PRTSP_MESSAGE response, uint16_t *port) {
  ...
  transport = getOptionContent(response->options, (char *)"Transport");
  if (transport == NULL) {
    return false;
  }
  ...
  if (CHANNEL_COUNT_FROM_AUDIO_CONFIGURATION(StreamConfig.audioConfiguration) == 2) {
    ...
    if (paramStart) {
      ...
      if (channelCount == 6 || channelCount == 8) {
        ...
        memcpy(&NormalQualityOpusConfig.mapping[4], &originalMapping.mapping[3], channelCount - 4);
      }
...
static bool parseUrlAddrFromRtspUrlString(const char *rtspUrlString, char *destination) {
  ...
  for (prefixLen = 2; rtspUrlScratchBuffer[prefixLen - 2] != 0 &&
       (rtspUrlScratchBuffer[prefixLen - 2] != '/' || rtspUrlScratchBuffer[prefixLen - 1] != '/');
       prefixLen++);
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo,int StreamCount) {
  int ret;
  ...
  currentSeqNumber = 1;
  hasSessionId = false;
  controlStreamId = APP_VERSION_AT_LEAST(7, 1, 431) ? (char *)"streamid=control/13/0" : (char *)"streamid=control/1/0";
...

  {
    ...
    if (StreamConfig.supportsHevc && strstr(response.payload, "sprop-parameter-sets=AAAAAU")) {
      ...
      if (StreamConfig.width > 4096 ||
        StreamConfig.height > 4096 ||
        StreamConfig.width2 > 4096 ||
        StreamConfig.height2 > 4096) {
        Limelog(
            "WARNING: Host PC doesn't support HEVC. Streaming at resolutions above 4K using H.264 will "
            "likely fail!\n");
      }
    }
...

  {
    ...
    if (!setupStream(&response, AppVersionQuad[0] >= 5 ? (char *)"streamid=audio/0/0" : (char *)"streamid=audio",
                     &error)) {
    ...
    if (LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48000) == 48000) {
      ...
      AudioPortNumber = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48000);
      Limelog("NGFW Audio port: %u\n", AudioPortNumber);
    }

    // Parse the Sunshine ping payload protocol extension if present
    memset(&AudioPingPayload, 0, sizeof(AudioPingPayload));
    pingPayload = getOptionContent(response.options, (char *)"X-SS-Ping-Payload");
    ...
    sessionId = getOptionContent(response.options, (char *)"Session");
  ...
  // Setup Video Stream
  {
    ...
    if (!setupStream(&response, AppVersionQuad[0] >= 5 ? (char *)"streamid=video/0/0" : (char *)"streamid=video",
                     &error)) {
    ...
    // Parse the Sunshine ping payload protocol extension if present
    pingPayload = getOptionContent(response.options, (char *)"X-SS-Ping-Payload");
    VideoStreamInstance->UpdatePingPayload(pingPayload);

    // Parse the video port out of the RTSP SETUP response
    uint16_t VideoPortNumber = 0;
    if (LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47998) == 47998) {
      ...
      VideoPortNumber = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47998);
      Limelog("NGFW Video port: %u\n", VideoPortNumber);
    }
    VideoStreamInstance->SetPortNumber(VideoPortNumber);
    freeMessage(&response);
  }
  // Setup Second Video Stream
  if (StreamCount > 1) {
    RTSP_MESSAGE response;
    int error = -1;
    char *pingPayload;

    if (!setupStream(&response, AppVersionQuad[0] >= 5 ? (char *)"streamid=video2/0/0" : (char *)"streamid=video2",
                     &error)) {
      Limelog("RTSP SETUP streamid=video request failed: %d\n", error);
      ret = error;
      goto Exit;
    ...
    // Parse the Sunshine ping payload protocol extension if present
    pingPayload = getOptionContent(response.options, (char *)"X-SS-Ping-Payload");
    VideoStreamInstance2->UpdatePingPayload(pingPayload);

    // Parse the video port out of the RTSP SETUP response
    uint16_t VideoPortNumber2 = 0;
    if (LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48001) == 48001) {
      if (!parseServerPortFromTransport(&response, &VideoPortNumber2)) {
        // Use the well known port if parsing fails
        VideoPortNumber2 = 48001;

        Limelog("Video2 port: %u (RTSP parsing failed)\n", VideoPortNumber2);
      } else {
        Limelog("Video port2: %u\n", VideoPortNumber2);
      }
    } else {
      VideoPortNumber2 = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_48001);
      Limelog("NGFW Video2 port: %u\n", VideoPortNumber2);
    }
  ...
  if (AppVersionQuad[0] >= 5) {
    ...
    [&](){
    if (LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47999) == 47999) {
      ...
      ControlPortNumber = LiGetPortFromPortFlagIndex(ML_PORT_INDEX_UDP_47999);
      Limelog("NGFW Control port: %u\n", ControlPortNumber);
    }
  ...
  // first video
  {
    ...
    if (!sendVideoAnnounce(&response, &error, "streamid=video")) {
  ...
  if (APP_VERSION_AT_LEAST(7, 1, 431)) {
    ...
    if (!playStream(&response, (char *)"/", &error)) {
  ...
  } else {
    {
      ...
      if (!playStream(&response, (char *)"streamid=video", &error)) {
...

    {
      ...
      if (!playStream(&response, (char *)"streamid=audio", &error)) {
