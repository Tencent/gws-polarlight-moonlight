...
std::atomic<int> _StreamNum_(1);
std::atomic<int> nTempStreamNum(1);
struct ControlInfo {
...
static ControlInfo C_I[2]; // for two video Streams

// we use idrFrameRequired_StreamId to avoid using dual threads here
// but we need to lock
static std::mutex idrFrameRequiredEvent_Lock;
static int idrFrameRequired_StreamId = -1;
static PLT_EVENT idrFrameRequiredEvent;

static bool stopping;
static bool disconnectPending;
static bool hdrEnabled;
static bool hdrEnabled2;
static int currentEnetSequenceNumber;
static bool encryptedControlStream;

static SS_HDR_METADATA hdrMetadata;

static std::mutex metricsMtx;
static bool canMetricsSend = false;

static PPLT_CRYPTO_CONTEXT encryptionCtx = nullptr;
static PPLT_CRYPTO_CONTEXT decryptionCtx = nullptr;
...
static const char *preconstructedPayloadsGen7Enc[] = {requestIdrFrameGen7Enc, startBGen5};
...
void ResetControlInfoPerStream(int streamId)
{
    int i = streamId - 1;
    LbqInitializeLinkedBlockingQueue(&C_I[i].invalidReferenceFrameTuples, 20);
    C_I[i].lastGoodFrame = 0;
    C_I[i].lastSeenFrame = 0;
    C_I[i].lossCountSinceLastReport = 0;
    C_I[i].intervalGoodFrameCount = 0;
    C_I[i].intervalTotalFrameCount = 0;
    C_I[i].intervalStartTimeMs = 0;
    C_I[i].lastIntervalLossPercentage = 0;
    C_I[i].lastConnectionStatusUpdate = CONN_STATUS_OKAY;
}
// Initializes the control stream
int initializeControlStream(int nStream) {
    stopping = false;
    PltCreateEvent(&idrFrameRequiredEvent);
    LbqInitializeLinkedBlockingQueue(&C_I[0].frameFecStatusQueue,
                                     8); // Limits number of frame status reports per periodic ping interval
    LbqInitializeLinkedBlockingQueue(&C_I[1].frameFecStatusQueue,
                                     8); // Limits number of frame status reports per periodic ping interval
    PltCreateMutex(&enetMutex);

    encryptedControlStream = APP_VERSION_AT_LEAST(7, 1, 431);
    // encryptedControlStream = false;//match with sunshine, shawn
    if (AppVersionQuad[0] == 3) {
        ...
        if (encryptedControlStream) {
            ...
            supportsIdrFrameRequest = false;
            // supportsIdrFrameRequest = true;//Shawn
        }
    }
...
void destroyControlStream(void) {
    ...
    encryptionCtx = nullptr;
    decryptionCtx = nullptr;

    PltCloseEvent(&idrFrameRequiredEvent);
    freeBasicLbqList(LbqDestroyLinkedBlockingQueue(&C_I[0].invalidReferenceFrameTuples));
    freeBasicLbqList(LbqDestroyLinkedBlockingQueue(&C_I[1].invalidReferenceFrameTuples));
    freeBasicLbqList(LbqDestroyLinkedBlockingQueue(&C_I[0].frameFecStatusQueue));
    freeBasicLbqList(LbqDestroyLinkedBlockingQueue(&C_I[1].frameFecStatusQueue));

    PltDeleteMutex(&enetMutex);
}

void queueFrameInvalidationTuple(int streamId, int startFrame, int endFrame) {
    LC_ASSERT(startFrame <= endFrame);

    if (isReferenceFrameInvalidationEnabled()) {
        PQUEUED_FRAME_INVALIDATION_TUPLE qfit;
        qfit = (PQUEUED_FRAME_INVALIDATION_TUPLE)malloc(sizeof(*qfit));
        if (qfit != NULL) {
            qfit->startFrame = startFrame;
            qfit->endFrame = endFrame;
            if (LbqOfferQueueItem(&C_I[streamId - 1].invalidReferenceFrameTuples, qfit, &qfit->entry) ==
                LBQ_BOUND_EXCEEDED) {
                // Too many invalidation tuples, so we need an IDR frame now
                Limelog("RFI range list reached maximum size limit\n");
                free(qfit);
                LiRequestIdrFrame(streamId);
            }
        } else {
            LiRequestIdrFrame(streamId);
        }
...
void LiRequestIdrFrame(int streamId) {
    // Any reference frame invalidation requests should be dropped now.
    // We require a full IDR frame to recover.
    freeBasicLbqList(LbqFlushQueueItems(&(C_I[streamId - 1].invalidReferenceFrameTuples)));

    // Request the IDR frame
    idrFrameRequiredEvent_Lock.lock();
    idrFrameRequired_StreamId = streamId;
    PltSetEvent(&idrFrameRequiredEvent);
    idrFrameRequiredEvent_Lock.unlock();
    Limelog("Request the IDR frame");
}

// Invalidate reference frames lost by the network
void connectionDetectedFrameLoss(int streamId, int startFrame, int endFrame) {
    queueFrameInvalidationTuple(streamId, startFrame, endFrame);
}

// When we receive a frame, update the number of our current frame
void connectionReceivedCompleteFrame(int streamId, int frameIndex) {
    C_I[streamId - 1].lastGoodFrame = frameIndex;
    C_I[streamId - 1].intervalGoodFrameCount++;
}

void connectionSendFrameFecStatus(int streamId, PSS_FRAME_FEC_STATUS fecStatus) {
    ...
    PQUEUED_FRAME_FEC_STATUS queuedFecStatus = (PQUEUED_FRAME_FEC_STATUS)malloc(sizeof(*queuedFecStatus));
    if (queuedFecStatus != NULL) {
        queuedFecStatus->fecStatus = *fecStatus;
        if (LbqOfferQueueItem(&C_I[streamId - 1].frameFecStatusQueue, queuedFecStatus, &queuedFecStatus->entry) ==
            LBQ_BOUND_EXCEEDED) {
...
void connectionSawFrame(int streamId, int frameIndex) {
    LC_ASSERT(!isBefore16(frameIndex, C_I[streamId - 1].lastSeenFrame));

    uint64_t now = PltGetMillis();
    if (now - C_I[streamId - 1].intervalStartTimeMs >= CONN_STATUS_SAMPLE_PERIOD) {
        if (C_I[streamId - 1].intervalTotalFrameCount != 0) {
            // Notify the client of connection status changes based on frame loss rate
            int frameLossPercent =
                100 - (C_I[streamId - 1].intervalGoodFrameCount * 100) / C_I[streamId - 1].intervalTotalFrameCount;
            if (C_I[streamId - 1].lastConnectionStatusUpdate != CONN_STATUS_POOR &&
                (frameLossPercent >= CONN_IMMEDIATE_POOR_LOSS_RATE ||
                 (frameLossPercent >= CONN_CONSECUTIVE_POOR_LOSS_RATE &&
                  C_I[streamId - 1].lastIntervalLossPercentage >= CONN_CONSECUTIVE_POOR_LOSS_RATE))) {
                ...
                C_I[streamId - 1].lastConnectionStatusUpdate = CONN_STATUS_POOR;
            } else if (frameLossPercent <= CONN_OKAY_LOSS_RATE &&
                       C_I[streamId - 1].lastConnectionStatusUpdate != CONN_STATUS_OKAY) {
                ListenerCallbacks.connectionStatusUpdate(CONN_STATUS_OKAY);
                C_I[streamId - 1].lastConnectionStatusUpdate = CONN_STATUS_OKAY;
            }

            C_I[streamId - 1].lastIntervalLossPercentage = frameLossPercent;
        }

        // Reset interval
        C_I[streamId - 1].intervalStartTimeMs = now;
        C_I[streamId - 1].intervalGoodFrameCount = C_I[streamId - 1].intervalTotalFrameCount = 0;
    }

    C_I[streamId - 1].intervalTotalFrameCount += frameIndex - C_I[streamId - 1].lastSeenFrame;
    C_I[streamId - 1].lastSeenFrame = frameIndex;
}

// When we lose packets, update our packet loss count
void connectionLostPackets(int streamId, int lastReceivedPacket, int nextReceivedPacket) {
    C_I[streamId - 1].lossCountSinceLastReport += (nextReceivedPacket - lastReceivedPacket) - 1;
}
...
static bool encryptControlMessage(PNVCTL_ENCRYPTED_PACKET_HEADER encPacket, PNVCTL_ENET_PACKET_HEADER_V2 packet) {
    ...
    return PltEncryptMessage(encryptionCtx, ALGORITHM_AES_GCM, 0, (unsigned char *)StreamConfig.remoteInputAesKey,
                             sizeof(StreamConfig.remoteInputAesKey), iv, sizeof(iv), (unsigned char *)(encPacket + 1),
                             AES_GCM_TAG_LENGTH, // Write tag into the space after the encrypted header
                             (unsigned char *)packet, encryptedSize,
                             ((unsigned char *)(encPacket + 1)) + AES_GCM_TAG_LENGTH,
                             &encryptedSize); // Write ciphertext after the GCM tag
}

// Caller must free() *packet on success!!!
static bool decryptControlMessageToV1(PNVCTL_ENCRYPTED_PACKET_HEADER encPacket, int encPacketLength,
                                      PNVCTL_ENET_PACKET_HEADER_V1 *packet, int *packetLength) {
    unsigned char iv[16] = {0};
    ...
    *packet = (PNVCTL_ENET_PACKET_HEADER_V1)malloc(plaintextLength);
    if (*packet == NULL) {
        return false;
    }

    if (!PltDecryptMessage(decryptionCtx, ALGORITHM_AES_GCM, 0, (unsigned char *)StreamConfig.remoteInputAesKey,
                           sizeof(StreamConfig.remoteInputAesKey), iv, sizeof(iv), (unsigned char *)(encPacket + 1),
                           AES_GCM_TAG_LENGTH, // The tag is located right after the header
                           ((unsigned char *)(encPacket + 1)) + AES_GCM_TAG_LENGTH,
                           plaintextLength, // The ciphertext is after the tag
...
static bool sendMessageEnet(short ptype, short paylen, const void *payload, bool reliable) {
    ...
    if (encryptedControlStream) {
        ...
        enetPacket = enet_packet_create(NULL, sizeof(*encPacket) + AES_GCM_TAG_LENGTH + sizeof(*packet) + paylen,
...
static bool sendMessageTcp(short ptype, short paylen, const void *payload) {
    ...
    packet = (PNVCTL_TCP_PACKET_HEADER)malloc(sizeof(*packet) + paylen);
    if (packet == NULL) {
        return false;
    }
...
bool LiSendMetricsAndLogs(unsigned char *data, int length) {
    std::lock_guard<std::mutex> guard(metricsMtx);
    if (!canMetricsSend) {
        return false;
    }

    const int type = 0x1010;
    bool ret = false;
    if (AppVersionQuad[0] >= 5) {
        ret = sendMessageEnet(type, length, data, true);
    } else {
        ret = sendMessageTcp(type, length, data);
    }
    return ret;
}
    ...
    while (!PltIsThreadInterrupted(&controlReceiveThread)) {
        ...
        if (event.type == ENET_EVENT_TYPE_RECEIVE) {
                ...
                if (ctlHdr->type == 0x0001) {
                    ...
                    if (event.packet->dataLength < sizeof(NVCTL_ENCRYPTED_PACKET_HEADER)) {
                        Limelog("Discarding runt encrypted control packet: %d < %d\n", event.packet->dataLength,
                                (int)sizeof(NVCTL_ENCRYPTED_PACKET_HEADER));
                        enet_packet_destroy(event.packet);
                        continue;
                    }
            ...
            if (ctlHdr->type == packetTypes[IDX_RUMBLE_DATA]) {
                ...
                BbInitializeWrappedBuffer(&bb, (char *)ctlHdr, sizeof(*ctlHdr), packetLength - sizeof(*ctlHdr),
                                          BYTE_ORDER_LITTLE);
                ...
                Limelog("hdr enableByte: %02x\n", enableByte);
                bool flag = (enableByte & 0x1 != 0);
                int displayIndex = 1;
                if (enableByte & 0x40) {
                    displayIndex = 2;
                    hdrEnabled2 = flag;
                ...
                ListenerCallbacks.setHdrMode(flag, displayIndex);
                ...
                if (packetLength >= 6) {
                    ...
                    case 0x80030023: // NVST_DISCONN_SERVER_TERMINATED_CLOSED
                        if (C_I[0].lastSeenFrame != 0 || C_I[1].lastSeenFrame != 0) {
    ...
    if (usePeriodicPing) {
        ...
        while (!PltIsThreadInterrupted(&lossStatsThread)) {
            ...
            if (IS_SUNSHINE()) {
                ...
                LC_ASSERT(peer != NULL);
                for (int i = 0; i < _StreamNum_.load(); i++) {
                    while (LbqPollQueueElement(&C_I[i].frameFecStatusQueue, (void **)&queuedFrameStatus) ==
                           LBQ_SUCCESS) {
                        // Send as an unreliable packet, since it's not a critical message
                        if (!sendMessageEnet(SS_FRAME_FEC_PTYPE, sizeof(queuedFrameStatus->fecStatus),
                                             &queuedFrameStatus->fecStatus, false)) {
        ...
        lossStatsPayload = (char *)malloc(payloadLengths[IDX_LOSS_STATS]);
        ...
        while (!PltIsThreadInterrupted(&lossStatsThread)) {
            for (int i = 0; i < 2; i++) {
                ...
                BbInitializeWrappedBuffer(&byteBuffer, lossStatsPayload, 0, payloadLengths[IDX_LOSS_STATS],
                                          BYTE_ORDER_LITTLE);
                BbPut32(&byteBuffer, C_I[i].lossCountSinceLastReport);
                BbPut32(&byteBuffer, LOSS_REPORT_INTERVAL_MS);
                BbPut32(&byteBuffer, 1000);
                BbPut64(&byteBuffer, C_I[i].lastGoodFrame);
                BbPut32(&byteBuffer, 0);
                BbPut32(&byteBuffer, 0);
                BbPut32(&byteBuffer, 0x14);

                // Send the message (and don't expect a response)
                if (!sendMessageAndForget(packetTypes[IDX_LOSS_STATS], payloadLengths[IDX_LOSS_STATS],
                                          lossStatsPayload)) {
...
static void requestIdrFrame(int sId) {
    ...
    if (!supportsIdrFrameRequest) {
        int64_t payload[3];
        // Form the payload
        if (C_I[sId - 1].lastSeenFrame < 0x20) {
            payload[0] = 0;
            payload[1] = LE64(C_I[sId - 1].lastSeenFrame);
        } else {
            payload[0] = LE64(C_I[sId - 1].lastSeenFrame - 0x20);
            payload[1] = LE64(C_I[sId - 1].lastSeenFrame);
        }
        // Shawn, add streamId
        payload[2] = sId;
        ...
        char payload[2] = {(char)sId, 0};
        // Send IDR frame request and read the response
        if (!sendMessageAndDiscardReply(packetTypes[IDX_REQUEST_IDR_FRAME] + sId - 1,
                                        payloadLengths[IDX_REQUEST_IDR_FRAME], payload)) {
            Limelog("Request IDR Frame: Transaction failed: %d\n", (int)LastSocketError());
            ListenerCallbacks.connectionTerminated(LastSocketFail());
            return;
        }
    ...
    Limelog("IDR frame request sent,streamId=%d\n", sId);
}

static void requestInvalidateReferenceFrames(int streamId, int startFrame, int endFrame) {
    int64_t payload[3];
    ...
    // Send the reference frame invalidation request and read the response
    if (!sendMessageAndDiscardReply(packetTypes[IDX_INVALIDATE_REF_FRAMES], payloadLengths[IDX_INVALIDATE_REF_FRAMES],
                                    payload)) {
...
static void invalidateRefFramesFunc(void *context) {
    LC_ASSERT(isReferenceFrameInvalidationEnabled());
    int streamId = (int)(size_t)context;
    Limelog("Called with invalidateRefFramesFunc streamId: %d\n", streamId);

    while (!PltIsThreadInterrupted(&C_I[streamId - 1].invalidateRefFramesThread)) {
        PQUEUED_FRAME_INVALIDATION_TUPLE qfit;
        int startFrame;
        int endFrame;

        // Wait for a reference frame invalidation request or a request to shutdown
        if (LbqWaitForQueueElement(&C_I[streamId - 1].invalidReferenceFrameTuples, (void **)&qfit) != LBQ_SUCCESS) {
        ...
        do {
            ...
            endFrame = qfit->endFrame;
            free(qfit);
        } while (LbqPollQueueElement(&C_I[streamId - 1].invalidReferenceFrameTuples, (void **)&qfit) == LBQ_SUCCESS);

        // Send the reference frame invalidation request
        requestInvalidateReferenceFrames(streamId, startFrame, endFrame);
    }
}
...
static void requestIdrFrameFunc(void *context) {
    while (!PltIsThreadInterrupted(&requestIdrFrameThread)) {
        ...
        idrFrameRequiredEvent_Lock.lock();
        int streamId = idrFrameRequired_StreamId;
        idrFrameRequired_StreamId = -1;
        ...
        // Request the IDR frame
        requestIdrFrame(streamId);
    }
}

// Stops the control stream
int stopControlStream(int nStream) {
    {
        std::lock_guard<std::mutex> guard(metricsMtx);
        canMetricsSend = false;
    }
    stopping = true;
    LbqSignalQueueShutdown(&C_I[0].invalidReferenceFrameTuples);
    LbqSignalQueueShutdown(&C_I[1].invalidReferenceFrameTuples);
    LbqSignalQueueShutdown(&C_I[0].frameFecStatusQueue);
    LbqSignalQueueShutdown(&C_I[1].frameFecStatusQueue);
    idrFrameRequiredEvent_Lock.lock();
    idrFrameRequired_StreamId = -1;
    PltSetEvent(&idrFrameRequiredEvent);
    idrFrameRequiredEvent_Lock.unlock();
    ...
    if (isReferenceFrameInvalidationEnabled()) {
        for (int i = 0; i < nStream; i++) {
            PltInterruptThread(&C_I[i].invalidateRefFramesThread);
            PltJoinThread(&C_I[i].invalidateRefFramesThread);
            PltCloseThread(&C_I[i].invalidateRefFramesThread);
        }
    }
...
int startControlStream(int nStream) {
    int err;
    ...
    // Send START A
    if (!sendMessageAndDiscardReply(packetTypes[IDX_START_A], payloadLengths[IDX_START_A],
    ...
    // Send START B
    if (!sendMessageAndDiscardReply(packetTypes[IDX_START_B], payloadLengths[IDX_START_B],
    ...
    if (isReferenceFrameInvalidationEnabled()) {
        for (int i = 1; i <= nStream; i++) {
            err = PltCreateThread("InvRefFrames", invalidateRefFramesFunc, (void *)i,
                                  &C_I[i - 1].invalidateRefFramesThread);
            if (err != 0) {
                stopping = true;
                idrFrameRequiredEvent_Lock.lock();
                idrFrameRequired_StreamId = -1;
                ...
                // the first one is OK, need to delete
                if (i > 1) {
                    PltInterruptThread(&C_I[0].invalidateRefFramesThread);
                    PltJoinThread(&C_I[0].invalidateRefFramesThread);
                    PltCloseThread(&C_I[0].invalidateRefFramesThread);
                }
...
bool LiGetCurrentHostDisplayHdrMode(int displayIndex) {
    return displayIndex == 2 ? hdrEnabled2: hdrEnabled;
}

bool LiGetHdrMetadata(PSS_HDR_METADATA metadata) {
    if (!IS_SUNSHINE() || !(hdrEnabled || hdrEnabled2)) {
        return false;
    }
