...
{
    auto pVS = (streamId <= 1) ? VideoStreamInstance : VideoStreamInstance2;
    return pVS->VDepack()->queueRtpPacket(decContext, queueEntry);
}
void notifyFrameLost(int streamId, unsigned int frameNumber, bool speculative)
{
    auto pVS = (streamId <= 1) ? VideoStreamInstance : VideoStreamInstance2;
    return pVS->VDepack()->notifyFrameLost(frameNumber, speculative);
}
bool LiWaitForNextVideoFrame(int streamId, VIDEO_FRAME_HANDLE* frameHandle, PDECODE_UNIT* decodeUnit)
{
    auto pVS = (streamId <= 1) ? VideoStreamInstance : VideoStreamInstance2;
    return pVS->VDepack()->LiWaitForNextVideoFrame(frameHandle, decodeUnit);
}
bool LiPollNextVideoFrame(int streamId, VIDEO_FRAME_HANDLE* frameHandle, PDECODE_UNIT* decodeUnit)
{
    auto pVS = (streamId <= 1) ? VideoStreamInstance : VideoStreamInstance2;
    return pVS->VDepack()->LiPollNextVideoFrame(frameHandle, decodeUnit);
}
...
public:
    ...
    void VideoReceiveThread() {
        ...
        while (!PltIsThreadInterrupted(&receiveThread)) {
            ...
            //解密
            if(vecCliGuuid.size() > 0 && err > 0)
            {
                std::vector<char> outvec;
                aes_decryptEx(buffer, receiveSize, vecCliGuuid.data(), outvec);
                memcpy(buffer, outvec.data(), receiveSize);
            }
            //Limelog("recvUdpSocket bufferSize =  %d\n",bufferSize);
