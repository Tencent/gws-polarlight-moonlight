...
typedef DWORD(WINAPI *PFNWLANOPENHANDLE)(DWORD dwClientVersion, PVOID pReserved, PDWORD pdwNegotiatedVersion,
                                         PHANDLE phClientHandle);
PFNWLANOPENHANDLE pfnWlanOpenHandle;

typedef DWORD(WINAPI *PFNWLANCLOSEHANDLE)(HANDLE hClientHandle, PVOID pReserved);
PFNWLANCLOSEHANDLE pfnWlanCloseHandle;
typedef DWORD(WINAPI *PFNWLANENUMINTERFACES)(HANDLE hClientHandle, PVOID pReserved,
                                             PWLAN_INTERFACE_INFO_LIST *ppInterfaceList);
PFNWLANENUMINTERFACES pfnWlanEnumInterfaces;
typedef VOID(WINAPI *PFNWLANFREEMEMORY)(PVOID pMemory);
PFNWLANFREEMEMORY pfnWlanFreeMemory;
typedef DWORD(WINAPI *PFNWLANSETINTERFACE)(HANDLE hClientHandle, CONST GUID *pInterfaceGuid, WLAN_INTF_OPCODE OpCode,
                                           DWORD dwDataSize, CONST PVOID pData, PVOID pReserved);
PFNWLANSETINTERFACE pfnWlanSetInterface;
...
int recvUdpSocket(SOCKET s, char *buffer, int size, bool useSelect) {
  ...
  do {
    if (useSelect) {
      ...
      // for each packet.
      err = (int)recvfrom(s, buffer, size, 0, NULL, NULL);
      if (err < 0 && (LastSocketError() == EWOULDBLOCK || LastSocketError() == EINTR || LastSocketError() == EAGAIN ||
#if defined(LC_WINDOWS)
...
int sendMtuSafe(SOCKET s, char *buffer, int size) {
  ...
  while (bytesSent < size) {
    int bytesToSend = size - bytesSent > TCPv4_MSS ? TCPv4_MSS : size - bytesSent;
...
int resolveHostName(const char *host, int family, int tcpTestPort, struct sockaddr_storage *addr,
  ...
  for (currentAddr = res; currentAddr != NULL; currentAddr = currentAddr->ai_next) {
    ...
    if (tcpTestPort != 0 && (res->ai_next != NULL || (tcpTestPort & TCP_PORT_FLAG_ALWAYS_TEST))) {
      SOCKET testSocket =
          connectTcpSocket((struct sockaddr_storage *)currentAddr->ai_addr, (SOCKADDR_LEN)currentAddr->ai_addrlen,
                           tcpTestPort & TCP_PORT_MASK, TEST_PORT_TIMEOUT_SEC);
...
void enterLowLatencyMode(void) {
  ...
  pfnWlanOpenHandle = (PFNWLANOPENHANDLE)GetProcAddress(WlanApiLibraryHandle, "WlanOpenHandle");
  pfnWlanCloseHandle = (PFNWLANCLOSEHANDLE)GetProcAddress(WlanApiLibraryHandle, "WlanCloseHandle");
  pfnWlanFreeMemory = (PFNWLANFREEMEMORY)GetProcAddress(WlanApiLibraryHandle, "WlanFreeMemory");
  pfnWlanEnumInterfaces = (PFNWLANENUMINTERFACES)GetProcAddress(WlanApiLibraryHandle, "WlanEnumInterfaces");
  pfnWlanSetInterface = (PFNWLANSETINTERFACE)GetProcAddress(WlanApiLibraryHandle, "WlanSetInterface");

  if (pfnWlanOpenHandle == NULL || pfnWlanCloseHandle == NULL || pfnWlanFreeMemory == NULL ||
      pfnWlanEnumInterfaces == NULL || pfnWlanSetInterface == NULL) {
