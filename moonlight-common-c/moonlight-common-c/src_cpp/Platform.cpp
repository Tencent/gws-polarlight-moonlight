...
void setThreadNameWin32(const char *name) {
    ...
    setThreadDescriptionFunc =
        (SetThreadDescription_t)GetProcAddress(GetModuleHandleA("kernel32.dll"), "SetThreadDescription");
...
bool PltIsThreadInterrupted(PLT_THREAD *thread) { return thread->cancelled; }

void PltInterruptThread(PLT_THREAD *thread) { thread->cancelled = true; }

#ifdef __WIIU__
static void thread_deallocator(OSThread *thread, void *stack) { free(stack); }
#endif
...
int PltCreateThread(const char *name, ThreadEntry entry, void *context, PLT_THREAD *thread) {
    ...
    if (!OSCreateThread(&thread->thread, ThreadProc, 0, (char *)ctx, stack_addr, stack_size, 0x10,
