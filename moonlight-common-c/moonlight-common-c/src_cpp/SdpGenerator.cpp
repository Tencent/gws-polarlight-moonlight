...
static int addAttributeBinary(PSDP_OPTION* head, char* name, const void* payload, int payloadLen) {
    ...
    option = (PSDP_OPTION)malloc(sizeof(*option) + payloadLen);
    if (option == NULL) {
        return -1;
    }
...
static int addGen3Options(PSDP_OPTION* head, char* addrStr) {
    ...
    err |= addAttributeString(head, (char*)"x-nv-general.serverAddress", addrStr);

    payloadInt = htonl(0x42774141);
    err |= addAttributeBinary(head,
                              (char*)"x-nv-general.featureFlags", &payloadInt, sizeof(payloadInt));

    payloadInt = htonl(0x41514141);
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[0].transferProtocol", &payloadInt, sizeof(payloadInt));
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[1].transferProtocol", &payloadInt, sizeof(payloadInt));
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[2].transferProtocol", &payloadInt, sizeof(payloadInt));
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[3].transferProtocol", &payloadInt, sizeof(payloadInt));

    payloadInt = htonl(0x42414141);
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[0].rateControlMode", &payloadInt, sizeof(payloadInt));
    payloadInt = htonl(0x42514141);
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[1].rateControlMode", &payloadInt, sizeof(payloadInt));
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[2].rateControlMode", &payloadInt, sizeof(payloadInt));
    err |= addAttributeBinary(head,
                              (char*)"x-nv-video[3].rateControlMode", &payloadInt, sizeof(payloadInt));

    err |= addAttributeString(head, (char*)"x-nv-vqos[0].bw.flags", "14083");

    err |= addAttributeString(head, (char*)"x-nv-vqos[0].videoQosMaxConsecutiveDrops", "0");
    err |= addAttributeString(head, (char*)"x-nv-vqos[1].videoQosMaxConsecutiveDrops", "0");
    err |= addAttributeString(head, (char*)"x-nv-vqos[2].videoQosMaxConsecutiveDrops", "0");
    err |= addAttributeString(head, (char*)"x-nv-vqos[3].videoQosMaxConsecutiveDrops", "0");
...
static int addGen4Options(PSDP_OPTION* head, char* addrStr) {
    ...
    sprintf(payloadStr, (char*)"rtsp://%s:%u", addrStr, RtspPortNumber);
    err |= addAttributeString(head, (char*)"x-nv-general.serverAddress", payloadStr);
...
static int addGen5Options(PSDP_OPTION* head) {
    ...
    //but we do twice on Video Init,so comment out
    ...
    if (APP_VERSION_AT_LEAST(7, 1, 431)) {
        ...
        sprintf(payloadStr, (char*)"%u", featureFlags);
        err |= addAttributeString(head, (char*)"x-nv-general.featureFlags", payloadStr);
        ...
        err |= addAttributeString(head, (char*)"x-nv-general.useReliableUdp", "13");
        ...
        // 1 data shard frame).
        err |= addAttributeString(head, (char*)"x-nv-vqos[0].fec.minRequiredFecPackets", "2");
        ...
        err |= addAttributeString(head, (char*)"x-nv-vqos[0].bllFec.enable", "0");
    }
    else {
        // We want to use the new ENet connections for control and input
        err |= addAttributeString(head, (char*)"x-nv-general.useReliableUdp", "1");
        err |= addAttributeString(head, (char*)"x-nv-ri.useControlChannel", "1");

        // When streaming 4K, lower FEC levels to reduce stream overhead
        if (StreamConfig.width >= 3840 && StreamConfig.height >= 2160) {
            err |= addAttributeString(head, (char*)"x-nv-vqos[0].fec.repairPercent", "5");
        }
        else {
            err |= addAttributeString(head, (char*)"x-nv-vqos[0].fec.repairPercent", "20");
        }
    }
    ...
    if (APP_VERSION_AT_LEAST(7, 1, 446) && (StreamConfig.width < 720 || StreamConfig.height < 540)) {
        ...
        // the host otherwise.
        err |= addAttributeString(head, (char*)"x-nv-vqos[0].drc.enable", "1");
        err |= addAttributeString(head, (char*)"x-nv-vqos[0].drc.tableType", "2");
    }
    else {
        // Disable dynamic resolution switching
        err |= addAttributeString(head, (char*)"x-nv-vqos[0].drc.enable", "0");
    }

    // Recovery mode can cause the FEC percentage to change mid-frame, which
    // breaks many assumptions in RTP FEC queue.
    err |= addAttributeString(head, (char*)"x-nv-general.enableRecoveryMode", "0");
...
static PSDP_OPTION getAttributesList(char*urlSafeAddr) {
    ...
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].clientViewportWd", payloadStr);
    sprintf(payloadStr, "%d", StreamConfig.height);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].clientViewportHt", payloadStr);
    sprintf(payloadStr, "%d", StreamConfig.fps);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].maxFPS", payloadStr);

    sprintf(payloadStr, "%d", StreamConfig.width2);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[1].clientViewportWd", payloadStr);
    sprintf(payloadStr, "%d", StreamConfig.height2);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[1].clientViewportHt", payloadStr);
    sprintf(payloadStr, "%d", StreamConfig.fps2);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[1].maxFPS", payloadStr);


    sprintf(payloadStr, "%d", StreamConfig.packetSize);
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].packetSize", payloadStr);

    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].rateControlMode", "4");

    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].timeoutLengthMs", "7000");
    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].framesWithInvalidRefThreshold", "0");
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].initialBitrateKbps", payloadStr);
        err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].initialPeakBitrateKbps", payloadStr);

        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bw.minimumBitrateKbps", payloadStr);
        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bw.maximumBitrateKbps", payloadStr);
    }
    else {
        if (StreamConfig.streamingRemotely == STREAM_CFG_REMOTE) {
            err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].averageBitrate", "4");
            err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].peakBitrate", "4");
        }

        sprintf(payloadStr, "%d", bitrate);
        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bw.minimumBitrate", payloadStr);
        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bw.maximumBitrate", payloadStr);
    }
    
    // FEC must be enabled for proper packet sequencing to be done by RTP FEC queue
    err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].fec.enable", "1");
    
    err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].videoQualityScoreUpdateTime", "5000");
    ...
    if (StreamConfig.streamingRemotely == STREAM_CFG_LOCAL) {
        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].qosTrafficType", "5");
        err |= addAttributeString(&optionHead, (char*)"x-nv-aqos.qosTrafficType", "4");
    }
    else {
        err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].qosTrafficType", "0");
        err |= addAttributeString(&optionHead, (char*)"x-nv-aqos.qosTrafficType", "0");
    }
    ...
    if (AppVersionQuad[0] >= 4) {
        ...
        err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].videoEncoderSlicesPerFrame", payloadStr);

        if (NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H265) {
            err |= addAttributeString(&optionHead, (char*)"x-nv-clientSupportHevc", "1");
            err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bitStreamFormat", "1");

            if (AppVersionQuad[0] >= 7) {
                // Enable HDR if requested
                if (StreamConfig.enableHdr) {
                    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].dynamicRangeMode", "1");
                }
                else {
                    err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].dynamicRangeMode", "0");
                }
            }
            ...
            if (!APP_VERSION_AT_LEAST(7, 1, 408)) {
                ...
                err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].encoderFeatureSetting", "0");
            }
        }
        else {
            
            err |= addAttributeString(&optionHead, (char*)"x-nv-clientSupportHevc", "0");
            err |= addAttributeString(&optionHead, (char*)"x-nv-vqos[0].bitStreamFormat", "0");
        ...
        if (AppVersionQuad[0] >= 7) {
            if (isReferenceFrameInvalidationEnabled()) {
                err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].maxNumReferenceFrames", "0");
            }
            else {
                ...
                err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].maxNumReferenceFrames", "1");
            }

            sprintf(payloadStr, "%d", StreamConfig.clientRefreshRateX100);
            err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].clientRefreshRateX100", payloadStr);
        }

        sprintf(payloadStr, "%d", audioChannelCount);
        err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.numChannels", payloadStr);
        sprintf(payloadStr, "%d", audioChannelMask);
        err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.channelMask", payloadStr);
        if (audioChannelCount > 2) {
            err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.enable", "1");
        }
        else {
            err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.enable", "0");
        }
    }
    ...
    if (AppVersionQuad[0] >= 7) {
        ...
        if (OriginalVideoBitrate >= HIGH_AUDIO_BITRATE_THRESHOLD && audioChannelCount > 2 &&
            ...
            err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.AudioQuality", "1");
            ...
            err |= addAttributeString(&optionHead, (char*)"x-nv-audio.surround.AudioQuality", "0");
        ...
        err |= addAttributeString(&optionHead, (char*)"x-nv-aqos.packetDuration", payloadStr);
    }
    else {
    ...
    if (AppVersionQuad[0] >= 7) {
        ...
        err |= addAttributeString(&optionHead, (char*)"x-nv-video[0].encoderCscMode", payloadStr);
    }
...
static int fillSdpTail(char* buffer) {
    LC_ASSERT(VideoStreamInstance->GetPortNumber() != 0);
    return sprintf(buffer,
                   "t=0 0\r\n"
                   "m=video %d  \r\n",
                   AppVersionQuad[0] < 4 ? 47996 : VideoStreamInstance->GetPortNumber());
}
...
char* getSdpPayloadForStreamConfig(int rtspClientVersion, int* length) {
    ...
    payload = (char*)malloc(MAX_SDP_HEADER_LEN + MAX_SDP_TAIL_LEN +
