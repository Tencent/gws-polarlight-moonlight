...
class VideoDepacketizer:
    public IVideoDepacketizer
{
public:
    VideoDepacketizer(const int id)
    {
        streamId = id;
    }
private:
    int streamId=1;//for dual streams

    PLENTRY nalChainHead =nullptr;
    PLENTRY nalChainTail =nullptr;
    ...
    void freeDecodeUnitList(PLINKED_BLOCKING_QUEUE_ENTRY entry) {
    ...
    bool getAnnexBStartSequence(PBUFFER_DESC current, PBUFFER_DESC startSeq) {
        if (current->length < 3) {
            return false;
        }
    ...
    virtual bool LiWaitForNextVideoFrame(VIDEO_FRAME_HANDLE* frameHandle, PDECODE_UNIT* decodeUnit) override{
    ...
    virtual void LiWakeWaitForVideoFrame() override {
        LbqSignalQueueUserWake(&decodeUnitQueue);
    }

    // Cleanup a decode unit by freeing the buffer chain and the holder
    virtual void LiCompleteVideoFrame(VIDEO_FRAME_HANDLE handle, int drStatus) override{
        PQUEUED_DECODE_UNIT qdu = (PQUEUED_DECODE_UNIT)handle;
        PLENTRY_INTERNAL lastEntry;
    ...
    bool isSeqReferenceFrameStart(PBUFFER_DESC buffer) {
    ...
    bool isAccessUnitDelimiter(PBUFFER_DESC buffer) {
        BUFFER_DESC startSeq;
    ...
    bool isSeiNal(PBUFFER_DESC buffer) {
        BUFFER_DESC startSeq;
    ...
    bool isPictureParameterSetNal(PBUFFER_DESC buffer) {
        BUFFER_DESC startSeq;
    ...
    void skipToNextNalOrEnd(PBUFFER_DESC buffer) {
        BUFFER_DESC startSeq;
    ...
    void skipToNextNal(PBUFFER_DESC buffer) {
    ...
    bool isIdrFrameStart(PBUFFER_DESC buffer) {
        BUFFER_DESC startSeq;
    ...
    void reassembleFrame(int frameNumber) {
        if (nalChainHead != NULL) {
            ...
            if (qdu != NULL) {
                ...
                qdu->decodeUnit.streamId = streamId;
                ...
                if (streamId == 2)
                {
                    //std::cout << "Get stream 2 frame" << std::endl;
                }
                ...
                qdu->decodeUnit.hdrActive = LiGetCurrentHostDisplayHdrMode(streamId);
                ...
                if (nalChainHead->bufferType != BUFFER_TYPE_PICDATA) {
                    ...
                    if (streamId == 1 && VideoStreamInstance) {
                        VideoStreamInstance->notifyKeyFrameReceived();
                    }
                    else if (streamId == 2 && VideoStreamInstance2) {
                        VideoStreamInstance2->notifyKeyFrameReceived();
                    }
                }
                else {
                ...
                if ((VideoCallbacks.capabilities & CAPABILITY_DIRECT_SUBMIT) == 0) {
                    LINKED_BLOCKING_QUEUE* decQueue = &decodeUnitQueue;
                    if (LbqOfferQueueItem(decQueue, qdu, &qdu->entry) == LBQ_BOUND_EXCEEDED) {
                        ...
                        freeDecodeUnitList(LbqFlushQueueItems(decQueue));
                ...
                connectionReceivedCompleteFrame(streamId, frameNumber);
    ...
    int getBufferFlags(char* data, int length) {
        BUFFER_DESC buffer;
    ...
    void queueFragment(PLENTRY_INTERNAL* existingEntry, char* data, int offset, int length) {
    ...
    void processRtpPayloadSlow(PBUFFER_DESC currentPos, PLENTRY_INTERNAL* existingEntry) {
    ...
    int isFirstPacket(uint8_t flags, uint8_t fecBlockNumber) {
    ...
    void processRtpPayload(PRTP_VIDEO_DECODE_CONTEXT decContext,
        PNV_VIDEO_PACKET videoPacket, int length,
        ...
        if (isBefore24(streamPacketIndex, U24(lastPacketInStream + 1)) ||
            ...
            if (waitingForIdrFrame) {
                ...
                connectionDetectedFrameLoss(streamId, startFrameNumber, frameIndex);
            }
            return;
        }
    ...
    [&](){
        ...
        if (firstPacket) {
            ...
            if (isBefore32(nextFrameNumber, frameIndex)) {
                if (nextFrameNumber + 1 == frameIndex) {
                    Limelog("Network dropped 1 frame (frame %d),streamId:%d\n", frameIndex - 1,streamId);
                }
                else {
                    Limelog("Network dropped %d frames (frames %d to %d),streamId:%d\n",
        ...
        if (firstPacket) {
            //decContext->streamId = *(uint16_t*)(currentPos.data + 1);
            //now change to use streamId in this class
            decContext->streamId = streamId;
            ...
            if (APP_VERSION_AT_LEAST(7, 1, 350)) {
                ...
                if (waitingForRefInvalFrame) {
                    connectionDetectedFrameLoss(streamId,startFrameNumber, frameIndex - 1);
                    waitingForRefInvalFrame = false;
                    waitingForNextSuccessfulFrame = false;
                }
            }
        ...
        if ((flags & FLAG_EOF) && fecCurrentBlockNumber == fecLastBlockNumber) {
            ...
            if (waitingForIdrFrame || waitingForRefInvalFrame) {
                ...
                if (waitingForIdrFrame) {
                    Limelog("Waiting for IDR frame,StreamId:%d\n",streamId);
                    ...
                    Limelog("Waiting for RFI frame,StreamId:%d\n", streamId);
                    connectionDetectedFrameLoss(streamId, startFrameNumber, frameIndex);
                }
    ...
    void notifyFrameLost(unsigned int frameNumber, bool speculative) {
        ...
        if (!waitingForIdrFrame) {
            ...
            connectionDetectedFrameLoss(streamId, startFrameNumber, frameNumber);
        }
    }

    // Add an RTP Packet to the queue
    void queueRtpPacket(PRTP_VIDEO_DECODE_CONTEXT decContext,
        PRTPV_QUEUE_ENTRY queueEntryPtr) {
        int dataOffset;
        ...
        processRtpPayload(decContext,
            (PNV_VIDEO_PACKET)(((char*)queueEntry.packet) + dataOffset),
...
{
    return new VideoDepacketizer(streamId);
}
void ReleaseVideoDepacketizer(IVideoDepacketizer* pInstance)
{
    delete (VideoDepacketizer*)pInstance;
}
