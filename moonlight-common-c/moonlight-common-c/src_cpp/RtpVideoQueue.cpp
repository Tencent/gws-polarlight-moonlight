...
static void reportFinalFrameFecStatus(int streamId, PRTP_VIDEO_QUEUE queue) {
    ...
    connectionSendFrameFecStatus(streamId, &fecStatus);
}

// newEntry is contained within the packet buffer so we free the whole entry by freeing entry->packet
static bool queuePacket(int streamId, PRTP_VIDEO_QUEUE queue, PRTPV_QUEUE_ENTRY newEntry, PRTP_PACKET packet,
                        int length, bool isParity, bool isFecRecovery) {
    ...
    if (!isFecRecovery) {
        ...
        } else if (queue->receivedOosData && newEntry->presentationTimeMs > queue->lastOosFramePresentationTimestamp +
                                                                                SPECULATIVE_RFI_COOLDOWN_PERIOD_MS) {
...
#define PACKET_RECOVERY_FAILURE()                                                                                      \
    ...
    Limelog("FEC recovery returned corrupt packet %d"                                                                  \
            " (frame %d)",                                                                                             \
            rtpPacket->sequenceNumber, queue->currentFrameNumber);                                                     \
    free(packets[i]);                                                                                                  \
    continue

// Returns 0 if the frame is completely constructed
static int reconstructFrame(int streamId, PRTP_VIDEO_QUEUE queue) {
    ...
    if (queue->pendingFecBlockList.count < neededPackets) {
        ...
        if (!queue->reportedLostFrame && !queue->receivedOosData) {
            ...
            if (queue->missingPackets > totalPackets - neededPackets) {
                notifyFrameLost(streamId, queue->currentFrameNumber, true);
                queue->reportedLostFrame = true;
            } else {
                // Assert that there are enough remaining packets to possibly recover this frame.
                LC_ASSERT(neededPackets - queue->pendingFecBlockList.count <=
                          U16(queue->bufferHighestSequenceNumber - queue->receivedHighestSequenceNumber));
            }
        }
    ...
    unsigned char **packets = (unsigned char **)calloc(totalPackets, sizeof(unsigned char *));
    unsigned char *marks = (unsigned char *)calloc(totalPackets, sizeof(unsigned char));
    ...
    packetBufferSize = receiveSize + sizeof(RTPV_QUEUE_ENTRY);
    ...
    for (i = 0; i < totalPackets; i++) {
        if (marks[i]) {
            packets[i] = (unsigned char *)malloc(packetBufferSize);
            if (packets[i] == NULL) {
                ret = -4;
    ...
    if (queue->bufferDataPackets != queue->receivedDataPackets) {
        ...
        reportFinalFrameFecStatus(streamId, queue);
    }
    ...
    for (i = 0; i < totalPackets; i++) {
        if (marks[i]) {
            ...
            if (ret == 0 && i < queue->bufferDataPackets) {
                ...
                nvPacket->multiFecBlocks = ((queue->multiFecLastBlockNumber << 2) | queue->multiFecCurrentBlockNumber)
                                           << 4;
...
#endif
                queuePacket(streamId, queue, queueEntry, rtpPacket, StreamConfig.packetSize + dataOffset, false, true);
...
static void submitCompletedFrame(int streamId, PRTP_VIDEO_DECODE_CONTEXT decContext, PRTP_VIDEO_QUEUE queue) {
    while (queue->completedFecBlockList.count > 0) {
        ...
        queueRtpPacket(streamId, decContext, entry);
    }
}

int RtpvAddPacket(int streamId, PRTP_VIDEO_DECODE_CONTEXT decContext, PRTP_VIDEO_QUEUE queue, PRTP_PACKET packet,
                  int length, PRTPV_QUEUE_ENTRY packetEntry) {
    ...
    if (queue->pendingFecBlockList.count == 0 || queue->currentFrameNumber != nvPacket->frameIndex ||
        ...
        if (queue->pendingFecBlockList.count != 0) {
            ...
            if (queue->multiFecLastBlockNumber != 0) {
                Limelog("Unrecoverable frame %d (block %d of %d): %d+%d=%d received < %d needed\n",
                        ...
                        queue->multiFecLastBlockNumber + 1, queue->receivedDataPackets, queue->receivedParityPackets,
                        queue->pendingFecBlockList.count, queue->bufferDataPackets);
                ...
                if (queue->currentFrameNumber == nvPacket->frameIndex) {
                    ...
                    if (!queue->reportedLostFrame) {
                        notifyFrameLost(streamId, queue->currentFrameNumber, false);
                        queue->reportedLostFrame = true;
                    }
        ...
        uint8_t expectedFecBlockNumber =
            (queue->currentFrameNumber == nvPacket->frameIndex ? queue->multiFecCurrentBlockNumber : 0);
        if (fecCurrentBlockNumber != expectedFecBlockNumber) {
            // Report the final status of the FEC queue before dropping this frame
            reportFinalFrameFecStatus(streamId, queue);

            Limelog("Unrecoverable frame %d: lost FEC blocks %d to %d\n", nvPacket->frameIndex,
                    expectedFecBlockNumber + 1, fecCurrentBlockNumber);
        ...
        if (queue->currentFrameNumber != nvPacket->frameIndex) {
            ...
            if (queue->currentFrameNumber + 1 != nvPacket->frameIndex || !queue->reportedLostFrame) {
                ...
                // frame it saw.
                notifyFrameLost(streamId, nvPacket->frameIndex - 1, false);
            }
        }
        ...
        connectionSawFrame(streamId, queue->currentFrameNumber);
        ...
        queue->bufferHighestSequenceNumber =
            U16(queue->bufferFirstParitySequenceNumber + queue->bufferParityPackets - 1);
        queue->multiFecCurrentBlockNumber = fecCurrentBlockNumber;
    ...
    if (!queuePacket(streamId, queue, packetEntry, packet, length,
                     !isBefore16(packet->sequenceNumber, queue->bufferFirstParitySequenceNumber), false)) {
        return RTPF_RET_REJECTED;
    } else {
        ...
        if (reconstructFrame(streamId, queue) == 0) {
            ...
            if (queue->multiFecCurrentBlockNumber < queue->multiFecLastBlockNumber) {
                ...
                submitCompletedFrame(streamId, decContext, queue);
