...
typedef struct _DECODE_UNIT {
    ...
    // Frame type
    int frameType;
    // shawn:add streamId passed from sunshine to support dual display
    int streamId;
...
#define MAKE_AUDIO_CONFIGURATION(channelCount, channelMask) (((channelMask) << 16) | (channelCount << 8) | 0xCA)
...
// This callback notifies the decoder that the stream is starting. No frames can be submitted before this callback
// returns.
typedef void (*DecoderRendererStart)(void);

// This callback notifies the decoder that the stream is stopping. Frames may still be submitted but they may be safely
// discarded.
typedef void (*DecoderRendererStop)(void);

// This callback performs the teardown of the video decoder. No more frames will be submitted when this callback is
// invoked.
...
typedef int (*AudioRendererInit)(int audioConfiguration, const POPUS_MULTISTREAM_CONFIGURATION opusConfig,
                                 void *context, int arFlags);

// This callback notifies the decoder that the stream is starting. No audio can be submitted before this callback
// returns.
typedef void (*AudioRendererStart)(void);

// This callback notifies the decoder that the stream is stopping. Audio samples may still be submitted but they may be
// safely discarded.
typedef void (*AudioRendererStop)(void);

// This callback performs the final teardown of the audio decoder. No additional audio will be submitted when this
// callback is invoked.
...
typedef void (*ConnListenerRumble)(unsigned short controllerNumber, unsigned short lowFreqMotor,
...
typedef void (*ConnListenerSetHdrMode)(bool hdrEnabled, int displayIndex);
...
//
void SetStremNum(int StreamNum);
...
const char *LiGetStageError(int stage);
...
#define ML_PORT_INDEX_UDP_48002 12
#define ML_PORT_INDEX_UDP_48001 13
#define ML_PORT_MAX (ML_PORT_INDEX_UDP_48001 + 1)
...
#define ML_PORT_KEY(s1, s2) s1##s2
...
unsigned int LiGetPortFlagsFromErrorCode(int errorCode);
...
// Set the port number for the specified port index
void SetLiGetPortFromPortFlagIndex(int portFlagIndex, int value);

int getTcpPortIndex(int value);

int getUdpPortIndex(int value);
