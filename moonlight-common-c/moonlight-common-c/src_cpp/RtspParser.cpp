...
static bool startsWith(const char *s, const char *prefix) { return strncmp(s, prefix, strlen(prefix)) == 0; }
...
int parseRtspMessage(PRTSP_MESSAGE msg, char *rtspMessage, int length) {
    ...
    char *messageBuffer = (char *)malloc(length + 1);
    ...
    sequence = getOptionContent(options, (char *)"CSeq");
    if (sequence != NULL) {
    ...
    if (flag == TYPE_REQUEST) {
        createRtspRequest(msg, messageBuffer, FLAG_ALLOCATED_MESSAGE_BUFFER | FLAG_ALLOCATED_OPTION_ITEMS, command,
                          target, protocol, sequenceNum, options, payload,
                          payload ? length - (int)(payload - messageBuffer) : 0);
    } else {
        createRtspResponse(msg, messageBuffer, FLAG_ALLOCATED_MESSAGE_BUFFER | FLAG_ALLOCATED_OPTION_ITEMS, protocol,
                           statusCode, statusStr, sequenceNum, options, payload,
                           payload ? length - (int)(payload - messageBuffer) : 0);
    }
...
void createRtspResponse(PRTSP_MESSAGE msg, char *message, int flags, char *protocol, int statusCode, char *statusString,
                        int sequenceNumber, POPTION_ITEM optionsHead, char *payload, int payloadLength) {
...
void createRtspRequest(PRTSP_MESSAGE msg, char *message, int flags, char *command, char *target, char *protocol,
                       int sequenceNumber, POPTION_ITEM optionsHead, char *payload, int payloadLength) {
...
char *serializeRtspMessage(PRTSP_MESSAGE msg, int *serializedLength) {
    ...
    serializedMessage = (char *)malloc(size);
